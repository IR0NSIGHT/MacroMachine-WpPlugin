...
{
    public static float[] interpolateCatmullRom(float[] positions, float[] handleStrengths, int[] handleToCurveIdx,
            int[] segmentLengths, float defaultVaule) {
        ...
        final float[] handlesWithSomeValues = supplementFirstAndLastTwoHandles(positions,
                RiverHandleInformation.INHERIT_VALUE, defaultVaule);
        ...
        float[] interpolatedPositions = interpolateFromHandles(ready.positions, ready.handleStrengths, tangents,
                ready.idcs);
     ...
     *
     * @param inHandles
     *            input flat list of handles. each idx = 1 handle
     * @param emptyMarker
     *            use this value to decide if a value in handle is "empty"
     * @return new array with set handles
     * @requires at least one handle value is set
     * @ensures first two and last two values are set, therefor all others can be
     *          interpolated, out.length = in.length
     */
    ...
    public static int[] estimateSegmentLengths(float[] xsPos, float[] ysPos, float[] xsHandlesOffset,
        ...
        for (int i = 0; i < xsPos.length - 1; i++) {
            ...
            int sizeOfSegment = (int) Math.ceil(CubicBezierSpline.estimateCurveSize(pos0, handle0, handle1, pos1, 2));
    ...
    /**
     * will take a handle array and index information will construct a interpolated
     * curve matching both
     *
     * @param positions
     *            flat list of handle values, each idx = 1 handle
     * @param curveIdxByHandle
     *            array where arr[handleIdx] = startIdx on curve, describes where
     *            each handle is on the curve
     * @return interpolated curve that fills unknown positions between the handles
     *         using catmull rom
     * @requires handles must not contain INHERIT values
     */
    public static float[] interpolateFromHandles(float[] positions, float[] handleStrengths, float[] tangents,
            int[] curveIdxByHandle) {
    ...
    /**
     * interpolates a segment of the given handle list using catmull rom. segment
     * starts at flatHandles[i] .. flatHandles[i+1]
     *
     * @param positions
     *            list of flat handle values. each idx = 1 handle
     * @param handleToCurve
     *            positions of handles on the curve
     * @param i
     *            index of segment start. can be 0 to length-3
     * @return interpolated segment between handle[i+1] and handle[i+2]
     */
    public static float[] interpolateSegment(float[] positions, float[] handleStrenghts, float[] tangents,
            int[] handleToCurve, int i) {
        assert i >= 0;
