...
public class CatMullRomInterpolation {
  public static float[] interpolateCatmullRom(
      float[] positions,
      float[] handleStrengths,
      int[] handleToCurveIdx,
      int[] segmentLengths,
      float defaultVaule) {
    if (positions.length < 2) return positions.clone();

    Predicate<float[]> arrayContainsValuesThatAreNotInheritVal =
        xs -> {
          ArrayList<Float> sorted = new ArrayList<>();
          for (float f : xs) sorted.add(f);
          sorted.removeIf(f -> f == RiverHandleInformation.INHERIT_VALUE);
          if (sorted.isEmpty()) System.out.println("whoopsie :(");
          return !sorted.isEmpty();
        };

    final float[] handlesWithSomeValues =
        supplementFirstAndLastTwoHandles(
            positions, RiverHandleInformation.INHERIT_VALUE, defaultVaule);
    assert arrayContainsValuesThatAreNotInheritVal.test(handlesWithSomeValues)
        : "handles MUST have at least one "
            + "value to be interpolable"
            + Arrays.toString(handlesWithSomeValues);

    // FIXME use float lengths everywhere
    float[] segmentLengthsFloat = new float[segmentLengths.length];
    for (int i = 0; i < segmentLengths.length; i++) segmentLengthsFloat[i] = segmentLengths[i];
    HandleAndIdcs in =
        new HandleAndIdcs(
            handlesWithSomeValues, handleToCurveIdx.clone(), segmentLengthsFloat, handleStrengths);
    ...
    assert ArrayUtility.isValidArray(tangents)
        : "some illegal values are in this float array: " + Arrays.toString(tangents);

    float[] interpolatedPositions =
        interpolateFromHandles(ready.positions, ready.handleStrengths, tangents, ready.idcs);
    assert ArrayUtility.isValidArray(interpolatedPositions)
        : "some illegal values are in this float array: " + Arrays.toString(tangents);

    assert interpolatedPositions.length == 1 + handleToCurveIdx[handleToCurveIdx.length - 1]
        : "interpolated " + "values " + "array is not as long as the whole curve";
  ...
  public static float[] tangentsFromPositions(float[] positions, float[] segmentLengths) {
    ...
    // calculate differences to previous and following neighbour, scale by parameter
    // distance to neighbour (segment length)
    for (int i = 0; i < positions.length; i++) {
      if (i > 0) tangents[i] += (positions[i] - positions[i - 1]) / segmentLengths[i - 1];
      if (i < positions.length - 1)
        tangents[i] += (positions[i + 1] - positions[i]) / segmentLengths[i];
      if (i > 0 && i < positions.length - 1) tangents[i] /= 2f;
    }
    return tangents;
  }

  /**
   * will prepare handles array so that it can be interpolated with catmull rom adds first two and
   * last two handels if not present, based on existing handles if no handles exist, uses default
   * value
   *
   ...
   */
  public static float[] supplementFirstAndLastTwoHandles(
      float[] inHandles, float emptyMarker, float defaultValue) {
  ...
  public static int[] estimateSegmentLengths(
      float[] xsPos, float[] ysPos, float[] xsHandlesOffset, float[] ysHandlesOffset) {
    ...
    for (int i = 0; i < xsPos.length - 1; i++) {
      ...
      int sizeOfSegment =
          (int) Math.ceil(CubicBezierSpline.estimateCurveSize(pos0, handle0, handle1, pos1, 2));
  ...
  /**
   * will take a handle array and index information will construct a interpolated curve matching
   * both
   *
   * @param positions flat list of handle values, each idx = 1 handle
   * @param curveIdxByHandle array where arr[handleIdx] = startIdx on curve, describes where each
   *     handle is on the curve
   ...
   */
  public static float[] interpolateFromHandles(
      float[] positions, float[] handleStrengths, float[] tangents, int[] curveIdxByHandle) {
    assert positions.length == curveIdxByHandle.length && tangents.length == positions.length
        : "both arrays" + " must be the same length as the represent the " + "same curveHandles";

    // assert curveIdxByHandle[0] == 0 : "curveIdxByHandle must represent the
    // complete curve.";
    ...
    for (int i = 0; i < positions.length - 1; i++) {
      float[] segment =
          interpolateSegment(positions, handleStrengths, tangents, curveIdxByHandle, i);
    ...
    for (int i = 0; i < curveIdxByHandle.length; i++) {
      ...
      int handleIdx = i;
      assert Math.abs(outCurvePositions[curveIdx] - positions[handleIdx]) < 0.001f
          : "a handle position is not " + "in the curve where it should be.";
    }
  ...
  public static boolean canBeInterpolated(float[] handles) {
    ...
    Arrays.sort(copy);
    int inheritIdx = Arrays.binarySearch(copy, RiverHandleInformation.INHERIT_VALUE);
    return handles.length >= 2 && inheritIdx < 0; // does not contain inherit value
  }

  /**
   * interpolates a segment of the given handle list using catmull rom. segment starts at
   * flatHandles[i] .. flatHandles[i+1]
   *
   ...
   */
  public static float[] interpolateSegment(
      float[] positions, float[] handleStrenghts, float[] tangents, int[] handleToCurve, int i) {
    assert i >= 0;
    assert i < positions.length - 1;
    assert positions.length == handleToCurve.length && positions.length == tangents.length
    ...
    for (int j = 0; j < length; j++) {
      float t = j * 1f / (length);
      float interpolatedV = CubicBezierSpline.calcuateCubicBezier(start, handle0, handle1, end, t);
