...
public class PathGeometryHelper implements BoundingBox {
  ...
  private PathGeometryHelper(
      Path path,
      TreeBoundingBox boundingBoxes,
  ...
  public PathGeometryHelper(Path path, ArrayList<Point> positions, double radius) {
    ...
    ArrayList<AxisAlignedBoundingBox2d> boundingBoxes =
        new ArrayList<>(PointUtils.toBoundingBoxes(this.curve, boxSizeFacctor, radius));
   ...
   *
   * @return map: key=points on continuous curve, value: list of points withint radius that are
   *     closest to the key
   */
  ...
  Collection<Point> allNearbyPoints() {
    ...
    for (Point c : curve) {
      allNearby.add(c);
      for (Integer f : factors) expandX(c, (int) radius, f, allNearby, false);
    }

    for (Point c : new HashSet<>(allNearby))
      for (Integer f : factors) expandX(c, (int) radius, f, allNearby, true);
    return allNearby;
  }
  ...
  public BoundingBox expand(double size) {
    return new PathGeometryHelper(
        path, treeBoundingBox.expand(size), curve, radius, segmentStartIdcs);
  }
