...
public class CubicBezierSpline {
  /**
   * calculate a cubic bezier curve connection start and endpoint with 2 control handles. returns a
   * path on the curve with @numPoints length. includes start and endpoint (?)
   *
   * @param startPoint
   * @param handle0
   * @param handle1
   ...
   * @return
   */
  public static float[] calculateCubicBezier(
      float startPoint, float handle0, float handle1, float endPoint, int numPoints) {
  ...
  public static float calcuateCubicBezier(
      float startPoint, float handle0, float handle1, float endPoint, float t) {
    double tSq = t * t, tCub = t * t * t;
    double x =
        Math.pow(1 - t, 3) * startPoint
   ...
   * @return
   */
  public static float estimateCurveSize(
      float[] pos0, float[] handle0, float[] handle1, float[] pos1, int positionDigits) {

    float[] xPoints = calculateCubicBezier(pos0[0], handle0[0], handle1[0], pos1[0], 5);
    float[] yPoints = calculateCubicBezier(pos0[1], handle0[1], handle1[1], pos1[1], 5);
     ...
     * float cont_net = PointUtils.getPositionalDistance(pos0, handle0,
     * positionDigits) + PointUtils.getPositionalDistance(handle1, handle0,
     * positionDigits) + PointUtils.getPositionalDistance(handle1, pos1,
     * positionDigits);
     */
    return (float) totalLength;
  }

  private static float[] getBezierHandle(
      float[] pointA, float[] pointB, float[] pointC, int positionDigits, float distance) {
    float[] out = new float[pointA.length];
    for (int n = 0; n < pointA.length; n++) {
      out[n] = (pointC[n] - pointA[n]) / 2f;
    }

    // normalize and scale positions based on distance, so the catmull rom spline
    // doesnt act crazy
