...
{
    /**
     * calculate a cubic bezier curve connection start and endpoint with 2 control
     * handles. returns a path on the curve with @numPoints length. includes start
     * and endpoint (?)
     *
     * @param startPoint
     * @param handle0
    ...
    public static float calcuateCubicBezier(float startPoint, float handle0, float handle1, float endPoint, float t) {
        ...
        double x = Math.pow(1 - t, 3) * startPoint + 3 * Math.pow(1 - t, 2) * t * handle0 + 3 * (1 - t) * tSq * handle1
                + tCub * endPoint;
        return (float) x;
    }
