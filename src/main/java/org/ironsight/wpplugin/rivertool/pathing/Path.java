...
public class Path implements Iterable<float[]> {
  ...
  public Path(List<float[]> handles, PointType type) {
  ...
  public static Path newFilledPath(int length, PointType type) {
  ...
  public static float[] interpolateWaterZ(ContinuousCurve curve, HeightDimension dim) {
    ...
    out[0] =
        Math.min(
            curve.getInfo(RiverHandleInformation.RiverInformation.WATER_Z)[0],
  ...
  public int indexOfPosition(float[] p) {
    for (int i = 0; i < handles.size(); i++) {
      if (PointUtils.arePositionalsEqual(
          p, handles.get(i), RiverHandleInformation.PositionSize.SIZE_2_D.value)) return i;
    }
    return -1;
  }
  ...
  public float[] getPreviousPoint(float[] point) throws IllegalAccessException {
    if (amountHandles() < 2)
      throw new IllegalAccessException(
          "can not find previous point on path with less than 2 points.");
  ...
  public Path insertPointAfter(float[] point, float[] newPosition) {
    ...
    if (idx == -1)
      throw new IllegalArgumentException(
          "can not find point " + Arrays.toString(point) + "in path");
  ...
  public int getClosestHandleIdxTo(float[] coord) throws IllegalAccessException {
    ...
    for (int i = 0; i < handles.size(); i++) {
      ...
      double distanceSq =
          PointUtils.getPositionalDistance(
              p, coord, RiverHandleInformation.PositionSize.SIZE_2_D.value);
