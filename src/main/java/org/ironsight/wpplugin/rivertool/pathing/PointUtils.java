...
public class PointUtils {
  ...
  public static ArrayList<AxisAlignedBoundingBox2d> toBoundingBoxes(
      ArrayList<Point> curve, int boxSizeFactor, double radius) {
  ...
  public static void drawCircle(
      Point center, int color, float radius, PaintDimension dimension, boolean dotted) {
  ...
  public static ArrayList<float[]> toPosition2DArray(ArrayList<float[]> points) {
    ...
    for (int i = 0; i < points.size(); i++) {
      ...
      float[] point2D =
          RiverHandleInformation.positionInformation(point[0], point[1], PointType.POSITION_2D);
      result.add(point2D);
    }
    return result;
  }
  ...
  public static float getPositionalDistance(float[] pointA, float[] pointB, int positionDigits) {
    ...
    assert !Float.isNaN(dist)
        : "distance between " + Arrays.toString(pointA) + " and " + Arrays.toString(pointB);
    return dist;
  }
