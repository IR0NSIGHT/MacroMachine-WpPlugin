...
public class TreeBoundingBox extends AxisAlignedBoundingBox2d {
  ...
  public TreeBoundingBox(AxisAlignedBoundingBox2d leftChild, AxisAlignedBoundingBox2d rightChild) {
    super(
        Arrays.asList(
            leftChild.minPoint, leftChild.maxPoint, rightChild.minPoint, rightChild.maxPoint),
        -1);
    this.leftChild = leftChild;
    this.rightChild = rightChild;
    this.sumChilds =
        (leftChild instanceof TreeBoundingBox ? ((TreeBoundingBox) leftChild).sumChilds : 1)
            + (rightChild instanceof TreeBoundingBox
                ? ((TreeBoundingBox) rightChild).sumChilds
                : 1);
  }

  public static TreeBoundingBox constructTree(
      Collection<AxisAlignedBoundingBox2d> neighbouringBoxes) {
  ...
  public void collectContainingAABBxsIds(Point p, Collection<Integer> out) {
    ...
    } else if (leftChild.contains(p)) out.add(leftChild.id);

    if (rightChild instanceof TreeBoundingBox) {
      ((TreeBoundingBox) rightChild).collectContainingAABBxsIds(p, out);
    } else if (rightChild.contains(p)) out.add(rightChild.id);
  }
