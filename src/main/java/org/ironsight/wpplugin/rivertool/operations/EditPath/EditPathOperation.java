...
/**
 * For any operation that is intended to be applied to the dimension in a particular location as
 * indicated by the user by clicking or dragging with a mouse or pressing down on a tablet, it makes
 * sense to subclass {@link MouseOrTabletOperation}, which automatically sets that up for you.
 *
 * <p>For more general kinds of operations you are free to subclass {@link AbstractOperation}
 * instead, or even just implement {@link Operation} directly.
 *
 * <p>There are also more specific base classes you can use:
 *
 * <ul>
 *   <li>{@link AbstractBrushOperation} - for operations that need access to the currently selected
 *       brush and intensity setting.
 *   <li>{@link RadiusOperation} - for operations that perform an action in the shape of the brush.
 *   <li>{@link AbstractPaintOperation} - for operations that apply the currently selected paint in
...
public abstract class EditPathOperation extends MouseOrTabletOperation
    implements PaintOperation, // Implement this if you
...
// classes already provide this
{
  ...
  /**
   * The globally unique ID of the operation. It's up to you what to use here. It is not visible to
   * the user. It can be a FQDN or package and class name, like here, or you could use a UUID. As
   * long as it is globally unique.
   */
  ...
  /**
   * Human-readable description of the operation. This is used e.g. in the tooltip of the operation
   * selection button.
   */
  static final String DESCRIPTION =
      "<html>Draw smooth, connected curves "
          + "with C1 continuity.<br>left click: "
          + "add "
          + "new"
          + " point "
          + "after selected<br>right click: delete selected<br>ctrl+click: "
          + "select "
          + "this "
          + "handle<br>shift+click: move "
          + "selected"
          + " "
          + "handle here</html>";
   ...
   * @param path
   */
  static void DrawPathLayer(Path path, ContinuousCurve curve, PaintDimension dim)
      throws IllegalAccessException {
    Path clone = path.clone();
    // nothing
    if (path.type == PointType.RIVER_2D) {
      ...
      wpApp
          ...
          .put(
              ...
              new AbstractAction() {
                @Override
                public void actionPerformed(ActionEvent e) {
                  if (currentState.indexSelection.getSelectedIdcs(false).length
                      == getSelectedPath().amountHandles()) // all
                    // are
                    // selected
      ...
      wpApp
          ...
          .put(
              ...
              new AbstractAction() {
                @Override
                public void actionPerformed(ActionEvent e) {
                  currentState.indexSelection.invertHandleSelection(
  ...
  private void overwriteSelectedPath(Path p, int[] newToOldMapping) {
    ...
    IndexSelection newSel =
        IndexSelection.translateToNew(currentState.indexSelection, newToOldMapping);

    this.currentState = new ToolHistoryState(p, newSel, currentState.pathId);
    try {
  ...
  /**
   * Perform the operation. For single shot operations this is invoked once per mouse-down. For
   * continuous operations this is invoked once per {@code delay} ms while the mouse button is down,
   * with the first invocation having {@code first} be {@code true} and subsequent invocations
   * having it be {@code false}.
   *
   ...
   *     this will always be {@code true}.
   * @param dynamicLevel The dynamic level (from 0.0f to 1.0f inclusive) to apply in addition to the
   *     {@code level} property, for instance due to a pressure sensitive stylus being used. In
  ...
  protected void tick(
    ...
    if (path.type == PointType.RIVER_2D)
      RiverHandleInformation.setValue(
          userClickedCoord,
    ...
    try {
      if (inverse) {
        ...
        if (shiftDown) {
          ...
          if (path.amountHandles() != 0) userDoDeleteAction();

        } else {
          if (path.amountHandles() != 0) userDoShiftPoint(userClickedCoord);
        }
      } else {
        // leftclick
        if (shiftDown) {
          if (path.amountHandles() != 0) userDoSelectAllFromCursor(userClickedCoord, path);
        } else if (ctrlDown) {
          userDoAddNewPoint(path, userClickedCoord);

        } else {
          if (path.amountHandles() != 0) userDoSelectPosition(userClickedCoord, path);
        }
      }

      assert getSelectedPath().amountHandles() == 0 || getCursorHandle() != null;

      assert getSelectedPath() == PathManager.instance.getPathBy(getSelectedPathId())
          : "unsuccessfull " + "setting " + "path in manager";
    } catch (Exception e) {
  ...
  private void userDoAddNewPoint(Path path, float[] userClickedCoord) {
    Path p =
        (path.amountHandles() == 0)
            ? path.addPoint(userClickedCoord)
  ...
  private int getHandleNear(float[] userClickedCoord, Path path) {
    ...
    try {
      if (path.amountHandles() != 0) {
        ...
        if (PointUtils.getPositionalDistance(
                closest, userClickedCoord, RiverHandleInformation.PositionSize.SIZE_2_D.value)
            < 50) {
          return clostestIdx;
        }
      }
    ...
    try {
      ...
      if (getCursorHandle() != null)
        PointUtils.drawCircle(
            PointUtils.getPoint2D(getCursorHandle()), COLOR_HANDLE, SIZE_SELECTED, paintDim, false);
      ...
      for (int selectedIdx : currentState.indexSelection.getSelectedIdcs(false)) {
        ...
        PointUtils.markPoint(
            PointUtils.getPoint2D(handle), COLOR_SELECTED, SIZE_MEDIUM_CROSS, paintDim);
      }
      ...
      for (int selectedIdx : currentState.indexSelection.getSelectedIdcs(true)) {
        if (currentState.indexSelection.isHandleSelected(selectedIdx + 1, true))
          for (int cIdx = handleToCurve[selectedIdx];
              cIdx < handleToCurve[selectedIdx + 1];
              cIdx++) {
  ...
  private class EditPathOptionsPanel extends OperationOptionsPanel<EditPathOptions> {
    ...
    @Override
    protected ArrayList<OptionsLabel> addComponents(
        EditPathOptions editPathOptions, Runnable onOptionsReconfigured) {
      ...
      comboBox.addActionListener(
          e -> {
            ...
            setSelectedPointIdx(
                getSelectedPath().amountHandles() == 0
                    ? -1
      ...
      submitNameChangeButton.addActionListener(
          e -> {
      ...
      if (getCursorHandle() != null) {
        if (getSelectedPath().type == PointType.RIVER_2D) {
          OptionsLabel[] riverInputs =
              RiverHandleInformation.Editor(
                  getCursorHandle(),
                  point -> {
                    try {
...

      {
        ...
        button1.addActionListener(
            e -> {
              ...
              JDialog dialog =
                  RiverHandleInformation.riverRadiusEditor(
                      c,
                      getSelectedPath(),
                      currentState.indexSelection.getCursorHandleIdx(),
                      p -> overwriteSelectedPath(p, Path.getOneToOneMapping(p)),
                      dim);
      ...
      { // subdivide segment
        {
          ...
          subdivideButton.addActionListener(
              e -> {
                Path p = getSelectedPath();
                ...
                int indexOffset = 0;
                Subdivide divider =
                    new HalfWaySubdivider(options.subdivisionRange, options.subdivisionRange, true);
                ...
                for (int handleIdx = 0; handleIdx < pathHandles.size(); handleIdx++) {
                  ...
                  if (currentState.indexSelection.isHandleSelected(handleIdx, true)
                    ...
                    ArrayList<float[]> newHandles =
                        Subdivide.subdivide(segment, (int) options.subdivisions, divider);
        ...
        }
        {
          SpinnerNumberModel model =
              new SpinnerNumberModel(options.subdivisionRange * 200f, 1, 100, 1f);
          OptionsLabel label =
              OptionsLabel.numericInput(
                  "subdivide range %",
