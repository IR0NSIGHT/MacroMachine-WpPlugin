...
public class ContinuousCurve {
  ...
  public ContinuousCurve(ArrayList<float[]> flatCurve, PointType type) {
    this.type = type;
  ...
  public static ContinuousCurve fromPath(Path path, HeightDimension dimension) {
    ...
    if (path.amountHandles() == 0) return new ContinuousCurve(new ArrayList<>(), path.type);

    // handles exist as flat lists, only true coords are used
    ArrayList<float[]> flatHandles =
        ArrayUtility.transposeMatrix(onlyNonInterpolateHandles(path.getHandles()));
    ...
    assert xsPos.length == ysPos.length
            && xsPos.length == xsHandleOffsets.length
            && ysPos.length == ysHandleOffsets.length
        : "positions and halndPerPositon must all be same length";

    int[] segmentLengths =
        CatMullRomInterpolation.estimateSegmentLengths(
            flatHandles.get(0), flatHandles.get(1), xsHandleOffsets, ysHandleOffsets);
    ...
    for (int n = 0; n < path.type.size; n++) {
      ...
      float defaultVal =
          n == RiverHandleInformation.RiverInformation.WATER_Z.idx + 2 ? 9999 : 5; // FIXME add
      // this info the
      // the
      // RiverInfroamtion
      // class
      float[] interpolated =
          CatMullRomInterpolation.interpolateCatmullRom(
              nthHandles, handleStrength, handleToCurveIdx, segmentLengths, defaultVal);
      interpolatedCurve.add(interpolated);
    }

    ArrayList<float[]> continuousFlats = makeContinuous(interpolatedCurve, path.type);
  ...
  private static ArrayList<float[]> onlyNonInterpolateHandles(ArrayList<float[]> handles) {
    ...
    for (float[] point : handles) {
      if (point[0] == RiverHandleInformation.INHERIT_VALUE
          || point[1] == RiverHandleInformation.INHERIT_VALUE) continue;
      out.add(point.clone());
    }
    return out;
  }
  ...
  public static float[] positionsToHandleOffsetCatmullRom(float[] positions) {
    ...
    for (int i = 1; i < handleOffsets.length - 1; i++) {
      handleOffsets[i] =
          (positions[i + 1] - positions[i - 1]) / 4f; // TODO this might cause overshooting if
  ...
  public static ArrayList<float[]> makeContinuous(
      ArrayList<float[]> flatCurveWithHoles, PointType type) {
    ArrayList<float[]> positions = ArrayUtility.transposeMatrix(flatCurveWithHoles);
    positions = roundHandles(positions, type);
  ...
  public static ArrayList<float[]> roundHandles(ArrayList<float[]> handles, PointType type) {
    MapPointAction a =
        new MapPointAction() {
          @Override
    ...
    for (float[] handle : rounded) {
      ...
      if (handle[0] == previous[0] && handle[1] == previous[1]) continue;
  ...
  /**
   * are all points of the curve connected to a previous neighbour on x or y axis -> no diagonal
   * jumps, no points spaced further than euclid distance 1
   *
   * @return
   */
  ...
  public Point[] getPositions2d() {
    ...
    if (curveLength() < 2) return positions;
  ...
  public ArrayList<float[]> getCurveAsPositions() {
    ArrayList<float[]> positions =
        ArrayUtility.transposeMatrix(flatCurve); // each entry is one position with x,y
    // and other data
    return positions;
  }
}
