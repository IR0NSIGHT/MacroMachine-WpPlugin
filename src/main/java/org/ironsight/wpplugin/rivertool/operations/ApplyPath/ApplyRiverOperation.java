...
public class ApplyRiverOperation extends MouseOrTabletOperation {
  ...
  static final String DESCRIPTION =
      "<html>Apply river to this world<br>Last selected path gets applied into the "
          + "world.<br>Potentially "
  ...
  private final StandardOptionsPanel optionsPanel =
      new StandardOptionsPanel(getName(), getDescription()) {
        @Override
  ...
  public static void applyRiverPath(
      Path path, ApplyPathOptions options, HeightDimension dimension, HeightDimension waterMap) {
    ...
    double totalSearchRadius =
        RiverHandleInformation.getValue(
                maxHandleValues, RiverHandleInformation.RiverInformation.RIVER_RADIUS)
            + RiverHandleInformation.getValue(
                maxHandleValues, RiverHandleInformation.RiverInformation.BEACH_RADIUS);
    PathGeometryHelper helper =
        new PathGeometryHelper(
            path, new ArrayList<>(Arrays.asList(curve.getPositions2d())), totalSearchRadius);
    HashMap<Point, Collection<Point>> parentage = helper.getParentage();

    HashMap<Point, Float> finalHeightmap = new HashMap<>();

    Function<float[], Float> riverDepthByDistance =
        (depthAndDist) -> {
          float a, b, c;
    ...
    for (int curveIdx = 0; curveIdx < curve.curveLength(); curveIdx++) {
      ...
      double riverRadius =
          curve.getInfo(RiverHandleInformation.RiverInformation.RIVER_RADIUS, curveIdx)
              * (1 + randomFluxAtIdx * randomPercent);
      double beachRadius =
          curve.getInfo(RiverHandleInformation.RiverInformation.BEACH_RADIUS, curveIdx);
      float waterHeight = waterZs[curveIdx];

      // Transition, becuase the gauss smoother can only do one value

      // FIXME: parentage is problematic: clostest point doenst guarentee to be the
      // right parent on rivers that
      // grow fast and are very curvy
      // -> point in the transition layer chose the wrong parent. instead we need to
      ...
      for (Point point : nearby) {
        ...
        if (distance <= riverRadius) {
          float[] params =
              new float[] {
                -curve.getInfo(RiverHandleInformation.RiverInformation.RIVER_DEPTH, curveIdx)
                    - 0.1f,
                (float) distance,
                (float) riverRadius
              };
    ...
    float maxTransition =
        (RiverHandleInformation.getValue(
            maxHandleValues, RiverHandleInformation.RiverInformation.TRANSITION_RADIUS));
  ...
  /**
   * collect all max values into a single point out[n] = Max(handles[all i][n])
   *
   * @param handles
   * @return
   */
  ...
  private static class ApplyPathOptionsPanel extends OperationOptionsPanel<ApplyPathOptions> {
    ...
    protected ArrayList<OptionsLabel> addComponents(
        ApplyPathOptions options, Runnable onOptionsReconfigured) {
      ArrayList<OptionsLabel> inputs = new ArrayList<>();

      inputs.add(
          OptionsLabel.numericInput(
              "random width",
              "each step the rivers radius will randomly increase or decrease. "
                  + "It will stay within +/- percent "
                  + "of the normal width.",
              new SpinnerNumberModel(options.randomFluctuate, 0, 100, 1f),
              w -> options.randomFluctuate = w.intValue(),
      ...
      inputs.add(
          OptionsLabel.numericInput(
              ...
              new SpinnerNumberModel(options.fluctuationSpeed, 0, 100, 1f),
              w -> options.fluctuationSpeed = w.intValue(),
              onOptionsReconfigured));
