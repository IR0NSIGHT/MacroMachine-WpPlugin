...
public class PathHistogram extends JPanel implements KeyListener {
  ...
  protected void paintComponent(Graphics g) {
    ...
    Stroke dottedHandle =
        new BasicStroke(
            3f / totalScale, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, dashPattern, 0);
    Stroke dottedGrid =
        new BasicStroke(
            1f / totalScale,
            ...
            0,
            new float[] {3f / totalScale, 2f / totalScale},
            0);

    int borderWidth = 100;
    g2d.translate(
        totalScale * (-userFocus.x) + borderWidth, totalScale * (userFocus.y) - borderWidth);
...

    {
      ...
      for (y = 0; y <= userFocus.y + 300; y += 50) {
        ...
        g2d.drawString(
            String.valueOf(y),
            userFocus.x - g2d.getFontMetrics().stringWidth(String.valueOf(y)),
            -y);
      }
      // vertical lines
      ...
      g2d.drawString(
          String.valueOf(y), userFocus.x - g2d.getFontMetrics().stringWidth(String.valueOf(y)), -y);

      g2d.drawString(
          String.format("ancor position %d,%d, zoom: %.2f", userFocus.x, userFocus.y, userZoom),
          userFocus.x,
          -userFocus.y + 2 * fontHeight);
      g2d.drawString(
          String.format("length: %d, handles: %d", curve.curveLength(), path.amountHandles()),
          userFocus.x,
          -userFocus.y + 3 * fontHeight);
      g2d.drawString(
          String.format(
              "highest water: %.0f, lowest water: %.0f",
    ...
    if (userFocus.y < oceanLevel) {
      ...
      g2d.drawString(
          "ocean level",
          userFocus.x - g2d.getFontMetrics().stringWidth("ocean level"),
          -oceanLevel);
    }
    ...
    for (int handleIdx = 0; handleIdx < path.amountHandles(); handleIdx++) {
      ...
      float curveHeightHandle =
          RiverHandleInformation.getValue(
              path.handleByIndex(handleIdx), RiverHandleInformation.RiverInformation.WATER_Z);
      ...
      if (userFocus.y < tHeight)
        g2d.drawString(
            text, pointCurveIdx - g.getFontMetrics().stringWidth(text) / 2, -tHeight - 10);

      if (curveHeightHandle > userFocus.y) {
        g2d.drawLine(
            pointCurveIdx, -userFocus.y, pointCurveIdx, -Math.round(curveHeightHandle)); // -(int)
        // tHeight
      }
          ...
          for (int i = 0; i < handleSelection.length; i++) {
            ...
            float[] newHandle =
                RiverHandleInformation.setValue(
                    handle, RiverHandleInformation.RiverInformation.WATER_Z, terrainHeight);
  ...
  private void changeHandleValue(float amount) {
    for (int handleIdx = 1; handleIdx < handleSelection.length - 1; handleIdx++) {
      ...
      float targetValue =
          RiverHandleInformation.sanitizeInput(
              RiverHandleInformation.getValue(
                      handle, RiverHandleInformation.RiverInformation.WATER_Z)
                  + amount,
              RiverHandleInformation.RiverInformation.WATER_Z);

      float[] newHandle =
          RiverHandleInformation.setValue(
              handle, RiverHandleInformation.RiverInformation.WATER_Z, targetValue);
