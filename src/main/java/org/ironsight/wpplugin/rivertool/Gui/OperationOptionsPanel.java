...
public abstract class OperationOptionsPanel<Options> extends JPanel {
  ...
  protected abstract ArrayList<OptionsLabel> addComponents(
      Options options, Runnable onOptionsReconfigured);
  ...
  private void initComponents(ArrayList<OptionsLabel> inputs) {
    ...
    for (OptionsLabel l : inputs) {
      for (JComponent c : l.getLabels()) horizontalGroup.addComponent(c);
      layout.setHorizontalGroup(horizontalGroup);
    }

    GroupLayout.SequentialGroup sequentialGroup = layout.createSequentialGroup();
    for (OptionsLabel l : inputs) {
      for (JComponent c : l.getLabels()) sequentialGroup.addComponent(c);
  ...
  public void removeComponentsAndCleanup(ArrayList<OptionsLabel> inputs) {
    if (inputs != null) {
      ...
      for (OptionsLabel l : inputs) {
        for (JComponent j : l.getLabels()) this.remove(j);
      }
