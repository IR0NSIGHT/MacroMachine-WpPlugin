...
public class KernelConvolution {
  ...
  public static float[] generateGaussianCurve(int size, float maxHeight, float mean, float stdDev) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      float value =
          (float)
              (maxHeight * Math.exp(-Math.pow((float) i - mean, 2) / (2 * Math.pow(stdDev, 2))));
  ...
  public void smoothGauss() {
    float[] kernel =
        radius <= 0
            ? new float[] {1}
            : generateGaussianCurve(2 * radius + 1, 10, radius, radius / 2f);
    float sum = 0;
    ...
    for (int i = 0; i < kernel.length; i++) {
      for (int j = 0; j < kernel.length; j++) sum += kernel[i] * kernel[j];
    }
    smoothPoints(kernel, sum);
  }
