...
public class BlockFacingDirectionIO implements IPositionValueGetter {
  ...
  public static int calculateSlopeFacing(int x, int y, Dimension dimension) {
    ...
    double angleDegrees =
        (Math.toDegrees(angleRadians) + 90) % 360; // ChatGPT made it look to the east, 90°
    // offset
    // corerctts it
  ...
  public int getValueAt(Dimension dim, int x, int y) {
    if (x <= dim.getLowestX() * TILE_SIZE
        || y <= dim.getLowestY() * TILE_SIZE
        || x >= (dim.getHighestX() + 1) * TILE_SIZE
        || y >= (dim.getHighestY() + 1) * TILE_SIZE) {
      return 0;
    }
