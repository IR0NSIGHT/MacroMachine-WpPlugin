...
/**
 * defines how a MappingAction influences its output. SET: overwrite value, dont care what it was
 * before INCREMENT: take existing value, add mapped value on top DECREMENT: increment but negative
 * MULTIPLY: see above DIVIDE: see above LIMIT_TO: take existing value and mapped value, choose the
 * lower number. effecitlvy a "min" operation AT_LEAST: take existing value and mapped value, choose
 * bigger number. "max" operation
 *
 * <p>note: not all action types make sense for all MappingActions, because they are math operations
 * but not all input/outputs are considered numbers by the user: Colors, ActionFilter (true/false)
 * etc.
 */
