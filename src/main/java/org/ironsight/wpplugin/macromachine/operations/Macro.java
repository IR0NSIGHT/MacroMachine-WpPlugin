...
/**
 * this class is a collection of MappingActions the action are ordered and executed in this order a
 * macro can be executed and will apply each of its nested action to the map macros can container
 * Actions or other Macros (nesting) recursion is technically possible but not allowed because there
 * is no way to detect infinite recursion.
 */
...
public class Macro implements SaveableAction {
  ...
  public Macro(
    ...
    if (activeActions == null) Arrays.fill(this.activeActions, true);
  }
   ...
   * @return new macro
   */
  public static Macro insertSaveableActionToList(
      Macro macro,
      SaveableAction item,
      Supplier<MappingAction> createNewAction,
      Consumer<MappingAction> updateAction,
      int[] targetRows,
    ...
    for (boolean b : macro.activeActions) activeUids.add(b);
    ...
    for (int i = 0; i < activeUids.size(); i++) active[i] = activeUids.get(i);
    Macro newMacro = macro.withUUIDs(ids, active);
    return newMacro;
  }

  public static List<MappingAction> macroToFlatActions(
      Macro macro, MacroContainer macroContainer, MappingActionContainer actionContainer) {
    List<UUID> steps = macro.collectActions(new LinkedList<>(), macroContainer, actionContainer);
    List<MappingAction> executionSteps =
        steps.stream().map(actionContainer::queryById).collect(Collectors.toList());
    return executionSteps;
  }

  public static Collection<ExecutionStatistic> applyMacroToDimension(
      ApplyAction.ApplicationContext context, Macro macro, ApplyActionCallback callback) {
    assert context != null;
    ...
    try {
      ...
      for (Layer l :
          getUsedLayers(executionSteps, context.internalLayerManager, System.err::println)) {
  ...
  public boolean equals(Object o) {
    ...
    Macro that = (Macro) o;
    return Arrays.equals(executionUUIDs, that.executionUUIDs)
        && Objects.equals(name, that.name)
        && Objects.equals(description, that.description)
        && Objects.equals(uid, that.uid)
        && Arrays.equals(activeActions, that.activeActions);
  }
  ...
  public Macro withRemovedItem(int itemIdx) {
    return new Macro(
        this.name,
        this.description,
        deleteAt(getExecutionUUIDs(), itemIdx),
        this.uid,
  ...
  public Macro withShiftedItem(int oldIdx, int newIdx) {
    ...
    return new Macro(this.name, this.description, newIds, this.uid, active);
  }
  ...
  public List<UUID> collectActions(
      List<UUID> actionList,
    ...
    for (UUID id : this.executionUUIDs) {
      ...
      if (!this.activeActions[idx++]) continue;
      if (action != null) { // macro
        ...
        if (action != null) actionList.add(action.getUid());
      }
    }
    return actionList;
  }
  ...
  public boolean containsNoUnknownActions(
      MappingActionContainer container, MacroContainer macroContainer) {
    for (UUID mappingUid : executionUUIDs) {
      ...
      else if (nestedMacro != null
          && !nestedMacro.containsNoUnknownActions(container, macroContainer)) return false;
      else continue;
    }
    return true;
  }
  ...
  public String toString() {
    return "Macro{"
        + "executionUUIDs="
        + Arrays.toString(executionUUIDs)
        + ", activeActions="
        + Arrays.toString(activeActions)
        + ", name='"
        + name
        + '\''
        + ", description='"
