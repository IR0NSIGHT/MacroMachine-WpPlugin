...
public class PerlinNoiseIO implements IPositionValueGetter, EditableIO {
    ...
    for (int value = 0; value <= 100; value++) {
      ...
      for (int x = 0; x < img.getWidth(); x++) {
        for (int y = 0; y < img.getHeight(); y++) {
          int posValue =
              io.getValueAt(
                  null, x * perlinSIze / img.getWidth(), y * perlinSIze / img.getHeight());
          float point = (float) posValue / 100;
          Color color;
          if (lower <= posValue && posValue <= upper) color = Color.RED;
          else color = new Color(point, point, point);
  ...
  private static final float[] octaveNormalizer =
      new float[] {
  ...
  public String[] getValueTooltips() {
    return new String[] {
      "the size of the noise in x/y direction.",
  ...
  public boolean equals(Object o) {
    ...
    return Float.compare(scale, that.scale) == 0
        && Float.compare(amplitude, that.amplitude) == 0
        && seed == that.seed
