...
public class FilterUtils
{
     ...
     * @param start
     * @param end
     * @param insideRangePass
     *            true=values between start and end will pass the filter, false:
     *            values outside of [start,end] will pass
     * @return
     */
    public static MappingAction asRangeFilter(MappingAction filter, int start, int end, boolean insideRangePass) {
        ...
        return filter.withNewPoints(
                new MappingPoint[]{new MappingPoint(start, insideValue), new MappingPoint(end, insideValue),
                        // put the caluclated ones at the end, bc they might get filtered out
                        new MappingPoint(start - 1, outsideValue), new MappingPoint(end + 1, outsideValue)});
    }
}
