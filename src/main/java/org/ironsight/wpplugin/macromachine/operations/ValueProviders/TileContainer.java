...
public class TileContainer {
  ...
  public void addAsValues(IPositionTileValueGetter getter, Dimension dim, boolean invert) {
    dim.getTiles().parallelStream()
        .forEach(
            t -> {
              int yOff = t.getY() * TILE_SIZE;
              int xOff = t.getX() * TILE_SIZE;
              if (!existsTile(t.getX(), t.getY())) return;
              ...
              for (int yPos = 0; yPos < TILE_SIZE; yPos++) {
                for (int xPos = 0; xPos < TILE_SIZE; xPos++) {
                  ...
                  if (invert) value = getter.getMaxValue() - value;
  ...
  public TileContainer(int width, int height, int minX, int minY, int defaultValue) {
    ...
    for (int j = 0; j < width; j++) {
      ...
      for (int i = 0; i < height; i++) tiles[j][i] = new IntegerTile(defaultValue);
    }
    offsetX = -minX;
    offsetY = -minY;
    this.width = width;
  ...
  private boolean invariant() {
    for (int j = 0; j < width; j++) {
      for (int i = 0; i < height; i++) if (tiles[j][i] == null) return false;
    }
    return true;
  }
  ...
  public void fillWithValue(int value) {
    assert invariant();
    for (int j = 0; j < width; j++) {
      for (int i = 0; i < height; i++) tiles[j][i].fillWith(value);
    }
    assert invariant();
  }
  ...
  public boolean existsTile(int tileX, int tileY) {
    ...
    if (indexX >= tiles.length) return false;
    if (indexY >= tiles[indexX].length) return false;
    if (tiles[indexX][indexY] == null) return false;
    return true;
  }
  ...
  private IntegerTile getTileAt(int pointX, int pointY) {
    ...
    assert pointY >= 0
        : pointY; // FIXME these fail in MacroConcurrentApplicatorTest testRunningMacro
  ...
  public void setValueAt(int x, int y, int value) {
    if (!existsTile(x >> TILE_SIZE_BITS, y >> TILE_SIZE_BITS)) return;
  ...
  public int getValueAt(int x, int y) {
    if (!existsTile(x >> TILE_SIZE_BITS, y >> TILE_SIZE_BITS)) return Integer.MAX_VALUE;
