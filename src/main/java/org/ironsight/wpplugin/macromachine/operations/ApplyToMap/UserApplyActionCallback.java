...
public class UserApplyActionCallback implements ApplyActionCallback {
  ...
  public void beforeEachAction(MappingAction a, Dimension dimension) {
    ...
    DebugUserInterface.BreakpointReaction breakpointReaction =
        DebugUserInterface.BreakpointReaction.WAIT;
    if (isDebug) {
      if (dimension != null) dimension.rememberChanges();
      try {
        while (!isActionAbort()
            && breakpointReaction == DebugUserInterface.BreakpointReaction.WAIT) {
