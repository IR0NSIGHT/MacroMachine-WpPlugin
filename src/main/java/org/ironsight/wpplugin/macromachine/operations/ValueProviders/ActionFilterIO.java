...
public class ActionFilterIO implements IPositionValueSetter, IPositionValueGetter {
  ...
  public int getValueAt(Dimension dim, int x, int y) {
    if (tileContainer == null) return PASS_VALUE; // pass by default
  ...
  public void prepareForDimension(Dimension dim) {
    if (tileContainer != null) return;
    if (debugMode) tileContainer = null;
    Rectangle rect = dim.getExtent();
    tileContainer =
        debugMode
            ? new DebugTileContainer(
                dim,
                Annotations.INSTANCE,
                new int[] {14 /* red */, 6 /* green */},
                rect.width,
                rect.height,
  ...
  /**
   * special case for action filter: check if the tile can be completly set to BLOCK early on, and
   * then be skipped fo rthe whole action. better performance than having to test all blocks in tile
   *
   * @param tile
   * @param filterAction
   * @return true: tile can be skipped, all actions already happened. false: gotta iterate all
   *     blocks in tile
   */
