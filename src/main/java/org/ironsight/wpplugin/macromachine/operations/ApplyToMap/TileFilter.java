...
public class TileFilter implements Serializable {
  ...
  private static passType testHeight(
      Tile tile, int minHeight, int maxHeight, FilterType filterByHeight) {
    ...
    else if (filterByHeight == FilterType.ONLY_ON
        && tile.getMinHeight() <= minHeight
        && tile.getMaxHeight() >= maxHeight) return passType.NO_BLOCKS;
    return passType.SOME_BLOCKS;
  }

  private static passType testTerrains(
      Tile tile, Set<Terrain> terrainIds, FilterType filterByTerrain) {
  ...
  public TileFilter withTerrain(FilterType filterByTerrain, Terrain... terrainIds) {
    ...
    this.terrainIds =
        Arrays.stream(terrainIds).collect(HashSet::new, HashSet::add, HashSet::addAll);
    return this;
  }
  ...
  private passType testExtent(Tile tile, FilterType filterByExtent) {
    switch (filterByExtent) {
      ...
      case ONLY_ON:
        {
          if (!filterExtent.contains(tile.getX(), tile.getY())) return passType.NO_BLOCKS;
        }
      case EXCEPT_ON:
        {
          if (filterExtent.contains(tile.getX(), tile.getY())) return passType.NO_BLOCKS;
  ...
  public passType testTile(Tile tile) {
    ...
    if (selection == passType.ALL_BLOCKS
        && terrains == passType.ALL_BLOCKS
        && height == passType.ALL_BLOCKS
