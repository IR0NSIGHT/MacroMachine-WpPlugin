...
public class RandomNoise implements IPositionValueGetter, IPositionTileValueGetter, EditableIO {
  ...
  public String[] getValueTooltips() {
    return new String[] {
      "seed", "on average, this 1 block out of every X blocks receives value '1'"
    };
  }
  ...
  public int getValueAt(Tile tile, int tileX, int tileY) {
    return getRandomPassForPos(
            tileX + tile.getX() * TILE_SIZE, tileY + tile.getX() * TILE_SIZE, chance, random)
        ? PASS
        : BLOCK;
  }
