...
public abstract class AbstractOperationContainer<T extends SaveableAction> {
  ...
  protected AbstractOperationContainer(
      Class<T> type, String filePath, String defaultFileResourcePath) {
    this.type = type;
    ...
    if (!saveFile.exists()) {
      try {
        ...
        try (InputStream inputStream =
            AbstractOperationContainer.class.getResourceAsStream(defaultFileResourcePath)) {
          if (inputStream != null) {
            Files.copy(inputStream, saveFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
            System.out.println(
                "Copied the default resource file from " + url + " to " + saveFilePath);
          } else {
        ...
        GlobalActionPanel.ErrorPopUpString(
            "Failed to copy the default resource file: " + e.getMessage());
      }
    } else {
  ...
  private void notify(UUID... mapping) {
    ...
    for (UUID obj : mapping)
      if (uidNotifies.containsKey(obj)) {
        for (Runnable r : uidNotifies.get(obj)) r.run();
      }
  }
  ...
  public synchronized T addMapping(UUID uuid) {
    T newMap;
    if (uuid != null) newMap = getNewAction(uuid);
    else newMap = getNewAction();
