...
public class ShadowMap {
  public static TileContainer calculateShadowMap(
      Rectangle extent, TerrainHeightIO heightIO, Dimension dim) {
    ...
    IntStream.iterate(minX, i -> i < maxX, i -> i + TILE_SIZE)
        .parallel()
        .forEach(
            x -> {
  ...
  public static TileContainer expandBinaryMask(
      BinaryLayerIO input, Dimension dimension, Rectangle extent, boolean invert) {
  ...
  public static TileContainer expandBinaryMask(TileContainer container, int incrementPerStep) {
    IntStream.iterate(container.getMinYPos(), i -> i < container.getMaxYPos(), i -> i + 1)
        .parallel()
        .forEach(
            y -> {
              // row by row
    ...
    IntStream.iterate(container.getMinXPos(), i -> i < container.getMaxXPos(), i -> i + 1)
        .parallel()
        .forEach(
            x -> {
              // row by row
              int[] columnXDist =
                  container.getValueColumn(
                      x); // consists of set values 0 .. x and unset values 0xFFFF
  ...
  public static int[] replaceValues(int[] row, int replacement, int original, boolean invert) {
    for (int i = 0; i < row.length; i++) {
      if (!invert && row[i] == original || invert && row[i] != original) row[i] = replacement;
    }
    return row;
  }
  ...
  public static int[] distanceFrom2Arrays(int[] xDistance, int[] yDistance) {
    ...
    for (int i = 0; i < dist.length; i++) {
      dist[i] =
          (int) Math.round(Math.sqrt(xDistance[i] * xDistance[i] + yDistance[i] * yDistance[i]));
    }
    return dist;
  }
  ...
  public static int[] expandBinaryLinearColumn(int[] horizontalDist) {
    ...
    for (int i = 0; i < dist.length; i++) {
      ...
      for (int j = 0; j < refPointIndices.length; j++) {
        ...
        if (distSq > distSqRefPoint) distSq = distSqRefPoint;
      }
  ...
  public static int[] calculateShadowFor(int[] heightmapColumn) {
    ...
    for (int i = heightmapColumn.length - 1; i >= 0; i--) {
      ...
      if (shadowHeight > terrainHeight) shadowMap[i] = shadowHeight - terrainHeight;
    }
    return shadowMap;
  }
}
