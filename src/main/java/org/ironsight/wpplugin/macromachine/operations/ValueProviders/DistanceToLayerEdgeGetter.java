...
public class DistanceToLayerEdgeGetter
    implements IPositionValueGetter, ILimitedMapOperation, EditableIO {
  ...
  protected DistanceToLayerEdgeGetter(
      boolean searchInwards, String name, String id, int maxDistance) {
  ...
  public IMappingValue instantiateFrom(Object[] data) {
    ...
    try {
      return new DistanceToLayerEdgeGetter(
          ((Integer) data[3]) == 1, (String) data[0], (String) data[1], (Integer) data[2]);
    } catch (Exception ex) {
   ...
   * @param x
   * @param y
   * @return: 0 = not in layer, 1-max = distance to edge, max+: somewhere inside layer but further
   *     away than max
   */
  @Override
  ...
  public void prepareRightBeforeRun(Dimension dimension, int[] tileX, int[] tileY) {
    ...
    if (tileX.length == 0) {
      ...
      extent =
          new Rectangle(
              Math.max(dimExtent.x, startX - expand),
    ...
    this.distanceMap =
        ShadowMap.expandBinaryMask(
            new BinaryLayerIO(layer, false), dimension, extent, searchInwards);
  }
  ...
  public String[] getValueTooltips() {
    return new String[] {
      "1 = search inwards of macroselection, 0 = search outwards of macroselection",
