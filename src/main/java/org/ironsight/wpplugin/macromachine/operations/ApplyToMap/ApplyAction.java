...
public class ApplyAction {
  ...
  public static class ApplicationContext {
    ...
    public ApplicationContext(
        ...
        LayerProvider layerManager,
        LayerProvider apiLayerManager,
        ActionFilterIO actionFilterIO) {
  ...
  public static ExecutionStatistic applyToDimensionWithFilter(
    ...
    while (t.hasNext()) {
      Tile tile = t.next();
      if (context.actionFilterIO.skipTile(tile.getX(), tile.getY())
          && // FIXME move static instance to be member
    ...
    for (int i = 0; i < tileX.length; i++) {
      ...
      if (callback.isActionAbort()) break;
      ...
      context
          .actionFilterIO
          .getTileContainer()
          .calculateMinMax(tile.getX() * TILE_SIZE, tile.getY() * TILE_SIZE);
      tilesVisitedCount++;
  ...
  public static ArrayList<ExecutionStatistic> applyExecutionSteps(
      ApplicationContext context, List<MappingAction> actions, ApplyActionCallback ui) {
    Dimension dim = context.dimension;
