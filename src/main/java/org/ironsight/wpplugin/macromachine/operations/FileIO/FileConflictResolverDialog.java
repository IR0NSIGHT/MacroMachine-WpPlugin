...
public class FileConflictResolverDialog extends JDialog {
  ...
  public FileConflictResolverDialog(
      JFrame owner, SaveableAction original, SaveableAction imported) {
  ...
  public FileConflictResolverDialog(
      JDialog owner, SaveableAction original, SaveableAction imported) {
    // Create a modal dialog
    super(owner, "Resolve " + "Import Conflict", true);
    init(owner, original, imported);
  }
  ...
  public static void main(String[] args) {
    ...
    SwingUtilities.invokeLater(
        () -> {
          ...
          FileConflictResolverDialog dialog =
              new FileConflictResolverDialog(frame, a, a.withName("Paint trees on cyan v2"));
  ...
  private void init(Component owner, SaveableAction original, SaveableAction imported) {
    ...
    JPanel originalPanel =
        (JPanel) new DisplayUnitRenderer(MacroTreePanel::isValidItem).renderFor(original, false);
    originalPanel.setBorder(BorderFactory.createTitledBorder("Existing:"));
    JPanel importedPanel =
        (JPanel) new DisplayUnitRenderer(MacroTreePanel::isValidItem).renderFor(imported, false);
    importedPanel.setBorder(BorderFactory.createTitledBorder("Imported:"));

    JLabel conflictLabel =
        new JLabel(
            "The imported "
                + (original instanceof Macro ? "macro" : "action")
    ...
    remeberButton.addActionListener(
        a -> {
