...
public class PreviewOperation extends AbstractBrushOperation {
  ...
  // HIGHLIGHT AREA
  public static final Layer annotationLayer =
      new AnnotationLayer(
          "macroMachine_3dpreview_annotationlayer",
          "3d " + "Preview",
          "Shows " + "what the 3d preview is currently " + "rendering",
          Layer.DataSize.BIT,
          true,
          65,
          '\0');
  ...
  protected void tick(
    ...
    Rectangle selectedSquare =
        new Rectangle(centreX - radius, centreY - radius, 2 * radius, 2 * radius);
    ...
    for (int x = centreXTile - tileRadiusMax; x <= centreXTile + tileRadiusMax; x++) {
      for (int y = centreYTile - tileRadiusMax; y <= centreYTile + tileRadiusMax; y++) {
        if (!dim.getExtent().contains(x, y)) continue;
        ...
        if (!selectedSquare.contains(tileCenterBlockX, tileCenterBlockY)) {
          System.out.printf(
              "reject chunk %d,%d because center %x %d not in extent %s",
              x, y, tileCenterBlockX, tileCenterBlockY, selectedSquare.toString());
          continue;
        }
