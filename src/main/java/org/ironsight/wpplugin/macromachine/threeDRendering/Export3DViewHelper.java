...
public class Export3DViewHelper {
  ...
  public static MinecraftWorldObject renderTileToSurfaceObject(
      Set<Tile> tiles, Dimension dimension) {
    WorldExportSettings settings = new WorldExportSettings();

    settings.setTilesToExport(
        tiles.stream().map(t -> new Point(t.getX(), t.getY())).collect(Collectors.toSet()));
    ...
    PreviewExporter exporter =
        new PreviewExporter(dimension.getWorld(), dimension.getWorld().getExportSettings());
    Map<Point, WorldRegion> worldRegionList = exporter.export(dimension);

    int tileMinX = Integer.MAX_VALUE,
        tileMinY = Integer.MAX_VALUE,
        tileMaxX = Integer.MIN_VALUE,
    ...
    for (Point regionPos : worldRegionList.keySet()) {
      ...
      for (int chunkX = 0; chunkX <= CHUNKS_PER_SIDE; chunkX++) {
        for (int chunkY = 0; chunkY <= CHUNKS_PER_SIDE; chunkY++) {
          Chunk chunk =
              region.getChunk(
                  chunkX + (int) regionPos.getX() * CHUNKS_PER_SIDE,
                  chunkY + (int) regionPos.getY() * CHUNKS_PER_SIDE);
          if (chunk != null) tileMaxHeight = Math.max(tileMaxHeight, chunk.getHighestNonAirBlock());
        }
      }
    }

    Box displayObjectBBX =
        new Box(
            tileMinX * TILE_SIZE,
    ...
    Point3i tileOffset =
        new Point3i(
            -displayObjectBBX.getWidth() / 2,
            -displayObjectBBX.getLength() / 2,
    ...
    final MinecraftWorldObject minecraftWorldObject =
        new MinecraftWorldObject(
            "Preview", displayObjectBBX, previewHeight, waterHeight, null, tileOffset);
    ...
    int blockMinX = Integer.MAX_VALUE,
        blockMinY = Integer.MAX_VALUE,
        blockMaxX = Integer.MIN_VALUE,
          ...
          if (chunk != null) {
            ...
            for (int x = 0; x < 16; ++x) {
              for (int z = 0; z < 16; ++z) {
                for (int y =
                        Math.min(
                            chunk.getHighestNonAirBlock(x, z),
                            dz + minecraftWorldObject.getVolume().getHeight() - 1);
                    y >= dz;
                    --y) {
    ...
    System.out.println(
        "block extents are:" + new Box(blockMinX, blockMaxX + 16, blockMinY, blockMaxY + 16, 0, 0));
