...
public class PreviewExporter extends JavaWorldExporter {
  ...
  public HashMap<Point, WorldRegion> export(Dimension dimension) {
    ...
    try {
      ExportSettings minimalExportSettings =
          new JavaExportSettings(
      ...
      int lowestRegionX = Integer.MAX_VALUE,
          highestRegionX = Integer.MIN_VALUE,
          lowestRegionZ = Integer.MAX_VALUE,
          highestRegionZ = Integer.MIN_VALUE;
      ...
      for (Point regionCoords : regions) {
        ...
        final WorldPainterChunkFactory chunkFactory =
            new WorldPainterChunkFactory(dimension, exporters, platform, maxHeight);
        ...
        Dimension ceiling =
            dimension
                .getWorld()
                .getDimension(
                    new Dimension.Anchor(
                        dimension.getAnchor().dim, dimension.getAnchor().role, true, 0));
        ;
        exportRegion(
            worldRegion,
            dimension,
  ...
  @Override
  public ExportResults exportRegion(
      MinecraftWorld minecraftWorld,
      Dimension dimension,
      Dimension ceiling,
      Point regionCoords,
      boolean tileSelection,
      Map<Layer, LayerExporter> exporters,
      Map<Layer, LayerExporter> ceilingExporters,
      ChunkFactory chunkFactory,
      ChunkFactory ceilingChunkFactory,
      ProgressReceiver progressReceiver) {
    return super.exportRegion(
        minecraftWorld,
        dimension,
        ceiling,
        regionCoords,
