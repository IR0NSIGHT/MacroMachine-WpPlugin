...
public class PreviewExporter extends JavaWorldExporter
{
    ...
    public HashMap<Point, WorldRegion> export(Dimension dimension) {
        ...
        try {
            ...
            for (Point regionCoords : regions) {
                ...
                WorldRegion worldRegion = new WorldRegion(regionCoords.x, regionCoords.y, minHeight, maxHeight,
                        platform);
                Dimension ceiling = dimension.getWorld()
                        .getDimension(
                                new Dimension.Anchor(dimension.getAnchor().dim, dimension.getAnchor().role, true, 0));;
                exportRegion(worldRegion, dimension, ceiling, regionCoords, false, exporters, ceilingExporters,
                        chunkFactory, ceilingChunkFactory, null);
    ...
    public ExportResults exportRegion(MinecraftWorld minecraftWorld, Dimension dimension, Dimension ceiling,
            ...
            Map<Layer, LayerExporter> ceilingExporters, ChunkFactory chunkFactory, ChunkFactory ceilingChunkFactory,
            ProgressReceiver progressReceiver) {
        return super.exportRegion(minecraftWorld, dimension, ceiling, regionCoords, tileSelection, exporters,
                ceilingExporters, chunkFactory, ceilingChunkFactory, progressReceiver);
    }
}
