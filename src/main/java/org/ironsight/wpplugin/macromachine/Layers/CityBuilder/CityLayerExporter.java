...
public class CityLayerExporter extends WPObjectExporter<CityLayer>
  ...
  public CityLayerExporter(
      Dimension dimension, Platform platform, ExporterSettings settings, CityLayer layer) {
    super(dimension, platform, settings, layer);
  }

  @Override
  public Fixup apply(
      Point3i location, int intensity, Rectangle exportedArea, MinecraftWorld minecraftWorld) {
    WPObject object = layer.getObjectForState(layer.getInformationAt(location.x, location.y));
    if (object == null) // layer says we dont want to place a building here.
    return null;
    final int height =
        ((object.getAttribute(ATTRIBUTE_HEIGHT_MODE) == HEIGHT_MODE_TERRAIN) ? location.z : 0)
  ...
  /**
   * Determines whether an object's attributes allow it to be placed at a certain location, and if
   * so where along the vertical axis.
   *
   * @return An indication of where along the vertical axis the object may be placed, which may be
   *     {@link Placement#NONE} if it may not be placed at all.
   */
  private Placement getPlacement(
    ...
    if (flooded && (spawnUnderWater || spawnUnderLava || spawnOnWater || spawnOnLava)) {
      ...
      Material materialUnderCoords =
          (z > minecraftWorld.getMinHeight()) ? minecraftWorld.getMaterialAt(x, y, z - 1) : AIR;
      if (object.getAttribute(ATTRIBUTE_SPAWN_ON_LAND)
          && (!materialUnderCoords.veryInsubstantial)) {
        return Placement.ON_LAND;
      } else if ((!object.getAttribute(ATTRIBUTE_NEEDS_FOUNDATION))
          && materialUnderCoords.veryInsubstantial) {
        return Placement.ON_LAND;
      }
    }
  ...
  @Override
  public List<Fixup> addFeatures(
      Rectangle area, Rectangle exportedArea, MinecraftWorld minecraftWorld) {
      ...
      for (int chunkY = area.y; chunkY < area.y + area.height; chunkY += 16) {
        ...
        for (int xx = chunkX; xx < chunkX + 16; xx++) {
          for (int yy = chunkY; yy < chunkY + 16; yy++) {
            WPObject object = getLayer().getObjectForState(getLayer().getInformationAt(xx, yy));
            if (object == null) continue;

            final int height =
                ((object.getAttribute(ATTRIBUTE_HEIGHT_MODE) == HEIGHT_MODE_TERRAIN)
                        ? (dimension.getIntHeightAt(xx, yy) + 1)
                        : 0)
                    + object.getAttribute(ATTRIBUTE_VERTICAL_OFFSET);

            final Placement placement =
                getPlacement(minecraftWorld, dimension, xx, yy, height, object);

            final int z =
                (placement == Placement.ON_LAND) ? height : dimension.getWaterLevelAt(xx, yy) + 1;
