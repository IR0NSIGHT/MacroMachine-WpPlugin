...
public class PathToolBackend {
  ...
  static PathResult getPathFromHandles(List<Point4f> handles, float handleStrength) {
    ...
    PointType type =
        new PointType(
            new RiverHandleInformation.RiverInformation[] {
              new RiverHandleInformation.RiverInformation(
                  0, "POINT_HEIGHT", "point height absolute", -10000, 10000),
    ...
    var curve =
        path.stream()
            .map(p -> p.data)
            .map(arr -> new Point4f(arr[0], arr[1], arr[2], arr[3]))
            .toList();
    var handlesToPathIndex = new HashMap<Point4f, Integer>(handles.size());
    for (int curveIdx = 0, handleIdx = 0;
        curveIdx < curve.size() && handleIdx < handles.size();
        curveIdx++) {
  ...
  static List<Point4f> getPathStartingAt(Point4f start, Collection<Point4f> path) {
    ...
    boolean add = false;
    for (var p : path) {
      if (p.equals(start)) add = true;
      if (add) outPath.add(p);
    }
    return outPath;
  }
   ...
   * @return
   */
  protected static ArrayList<Point4f> getSubPathFor(
      Point2i start, Point2i end, List<Point4f> path, float radiusMultiplier) {
    ArrayList<Point4f> subPath = new ArrayList<>();
    for (var pathPoint : path) {
      int radius = (int) Math.ceil(pathPoint.w * radiusMultiplier);
      if (contains(
          pathPoint,
          new Point2i(start.x - radius, start.y - radius),
          new Point2i(end.x + radius, end.y + radius))) subPath.add(pathPoint);
    }
    return subPath;
  }
  ...
  protected static FloatTile applyToTile(
      FloatTile heightInputTile,
      FloatTile paintOutput,
      Point3i tilePos,
      CrossSectionShape filterCrossSection,
      ArrayList<Point4f> subPath,
    ...
    for (var p : subPath) tree.insert(new Point2i(p.x, p.y));
    ...
    for (int xx = 0; xx < TILE_SIZE; xx++)
      for (int yy = 0; yy < TILE_SIZE; yy++) {
        // find closest point
        var query =
            new Point2i((tilePos.x << TILE_SIZE_BITS) + xx, (tilePos.y << TILE_SIZE_BITS) + yy);
        ...
        if (closest == null || distance > closest.w * transitionMultiplier) {
          ...
          filterStrength =
              filterStrengthFor(
                  (float) distance, closest.w, transitionMultiplier, filterCrossSection);
  ...
  protected static float filterStrengthFor(
      float distance,
      float maxDistance,
      float transitionMultiplier,
      CrossSectionShape filterCrossSection) {
