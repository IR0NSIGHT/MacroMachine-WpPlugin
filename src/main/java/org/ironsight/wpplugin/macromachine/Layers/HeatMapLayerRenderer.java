...
/**
 * The renderer is responsible for painting the layer in the editor view. There are various
 * implementations provided, or you can implement your own. Note that the renderer has to be of a
 * type corresponding to the data size of the layer (so it must implement {@link BitLayerRenderer},
 * {@link NibbleLayerRenderer} or {@link ByteLayerRenderer}.
 *
 * <p>The {@link Layer} by default finds the renderer by looking for a class in the {@code renderer}
 * subpackage and with the word {@code Renderer} appended to its own class name. In that case the
 * renderer must have a default (public, no arguments) constructor. You could also override the
 * {@link Layer#getRenderer()} method if you want to do something more complicated.
 *
 * <p>This demo uses the simplest provided implementation to create a very simple solid colour-based
 * renderer. Another provided implementation you could look at is {@link ColouredPatternRenderer},
 * which allows you to specify a simple pattern.
 *
...
public class HeatMapLayerRenderer extends TransparentColourRenderer {
  ...
  public int getPixelColour(int x, int y, int underlyingColour, int value) {
    int[] colors = {
      0xff2400, 0xff6000, 0xff9a00, 0xffd700, 0xebff00, 0xa5ff00, 0x62ff00, 0x18ff00, 0x00ff25,
