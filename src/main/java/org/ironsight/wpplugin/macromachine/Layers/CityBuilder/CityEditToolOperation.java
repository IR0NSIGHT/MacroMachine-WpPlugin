...
public class CityEditToolOperation extends AbstractBrushOperation
    implements PaintOperation, KeyEventDispatcher {
  ...
  public CityEditToolOperation() {
    ...
    Toolkit.getDefaultToolkit()
        .addAWTEventListener(
            e -> {
              if (e instanceof MouseWheelEvent ev
                  && isActive()
                  && (ev.getModifiersEx() & InputEvent.SHIFT_DOWN_MASK) != 0) {
                if (ev.getComponent()
                    .equals(
                        SwingUtilities.getDeepestComponentAt(
                            ev.getComponent(), ev.getX(), ev.getY()))) { // fire
                  // only
                  // once
  ...
  public static void updateInstance() {
    if (instance != null) instance.updatePanel();
  }
  ...
  public static UndoManager getUndoManager(Dimension obj)
  ...
  public boolean dispatchKeyEvent(KeyEvent e) {
    if (e.getID() == KeyEvent.KEY_PRESSED) {
      if (!isActive() || getDimension() == null) return false;
      if (e.isShiftDown() || e.isControlDown() || e.isAltDown() || e.isMetaDown()) return false;
      try {
        if (!getDimension().isEventsInhibited()) getDimension().setEventsInhibited(true);
        switch (e.getKeyCode()) {
          case KeyEvent.VK_W:
        ...
        if (getDimension().isEventsInhibited()) getDimension().setEventsInhibited(false);
      }
    }
  ...
  private CityLayer getSelectedLayer() {
    if (getPaint() instanceof LayerPaint layerPaint
        && layerPaint.getLayer() instanceof CityLayer cityLayer) {
      return cityLayer;
    }
    return null;
  }
  ...
  private void setSelectedObjectIndex(int index) {
    if (index == currentState.objectIndex) return;
  ...
  private void onMoveAt(int centreX, int centreY, CityLayer cityLayer) {
    if (lastCentreX == Integer.MAX_VALUE || lastCentreY == Integer.MAX_VALUE) return;
  ...
  protected void brushChanged(Brush newBrush) {
    ...
    if (newBrush instanceof RotatedBrush)
      setRotation(
          CityLayer.Direction.fromCompass((((RotatedBrush) getBrush()).getDegrees() + 360) % 360));
    else setRotation(CityLayer.Direction.NORTH);
  }
  ...
  private void setRotation(CityLayer.Direction rotation) {
    if (rotation == this.currentState.rotation) return;
  ...
  private void init() {
    ...
    randomSelectCheckBox.addActionListener(
        l -> this.isAutoRandomSelect = randomSelectCheckBox.isSelected());
    ...
    JLabel previewPanel =
        new JLabel() {
          ...
          public void paintComponent(Graphics g) {
            ...
            Image original =
                Objects.requireNonNull(getSelectedLayer()).getSchematicImage(currentState);
            if (original == null) return;
            int scale = Math.max(100, getHeight()) / original.getHeight(null);
            Image img =
                original.getScaledInstance(
                    original.getWidth(null) * scale,
                    original.getHeight(null) * scale,
  ...
  private void onObjectStateChanged() {
    ...
    if (selectedObjectIndex < 0 || selectedObjectIndex >= layer.getObjectList().size()) return;
