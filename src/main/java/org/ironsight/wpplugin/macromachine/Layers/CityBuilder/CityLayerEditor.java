...
public class CityLayerEditor extends AbstractLayerEditor<CityLayer>
  ...
  private static final org.slf4j.Logger logger =
      org.slf4j.LoggerFactory.getLogger(CityLayerEditor.class);
  ...
  public static void main(String[] args) {
    ...
    EditLayerDialog dlg =
        new EditLayerDialog(parent, DefaultPlugin.JAVA_ANVIL_1_19, new CityLayer("test", "test"));
    dlg.setVisible(() -> {});
  }
  ...
  public CityLayer createLayer() {
    CityLayer cityLayer =
        new CityLayer(
            "My City Layer", "Custom (e.g. bo2, bo3, nbt, schem and/or schematic) objects");
    return cityLayer;
  }
  ...
  private void addFilesOrDirectory() {
    ...
    if ((config.getCustomObjectsDirectory() != null)
        && config.getCustomObjectsDirectory().isDirectory()) {
    ...
    CustomObjectManager.UniversalFileFilter fileFilter =
        CustomObjectManager.getInstance().getFileFilter();
    fileChooser.setFileFilter(fileFilter);
    if (doWithoutExceptionReporting(() -> fileChooser.showOpenDialog(this))
        == JFileChooser.APPROVE_OPTION) {
      prefs.put(
          LAST_DIR_KEY, fileChooser.getSelectedFile().getParent()); // remember path for next use
      ...
      if (selectedFiles.length > 0) {
        ...
        for (File selectedFile : selectedFiles) {
          if (selectedFile.isDirectory()) {
            ...
            if (files == null) {
              beepAndShowError(
                  this,
                  selectedFile.getName() + " is not a directory or it cannot be read.",
        ...
        if (checkForNameOnlyMaterials && (!nameOnlyMaterialsNames.isEmpty())) {
          ...
          if (nameOnlyMaterialsNames.size() > 4) {
            message =
                format(
                    "One or more added objects contain block types that are\n"
  ...
  private void addFile(
      boolean checkForNameOnlyMaterials, Set<String> nameOnlyMaterialsNames, File file) {
    try {
      ...
      JOptionPane.showMessageDialog(
          this,
          e.getMessage() + " while loading " + file.getName() + "; it was not added",
          "Illegal Argument",
      ...
      JOptionPane.showMessageDialog(
          this,
          "I/O error while loading " + file.getName() + "; it was not added",
          "I/O Error",
          JOptionPane.ERROR_MESSAGE);
    }
  }
  ...
  private void reloadObjects() {
    ...
    if ((noFiles.length() > 0) || (notFound.length() > 0) || (errors.length() > 0)) {
      ...
      JOptionPane.showMessageDialog(
          this, message, "Not All Files Reloaded", JOptionPane.ERROR_MESSAGE);
    } else {
  ...
  private void editObjects() {
    ...
    EditObjectAttributes dialog =
        new EditObjectAttributes(
            SwingUtilities.getWindowAncestor(this), selectedObjects, colourScheme);
  ...
  private void initComponents() {
    ...
    buttonReloadAll.setIcon(
        new javax.swing.ImageIcon(
            getClass()
                .getResource(
                    "/org/pepsoft/worldpainter/icons/arrow_rotate_clockwise.png"))); // NOI18N
    ...
    buttonEdit.setIcon(
        new javax.swing.ImageIcon(
            getClass().getResource("/org/pepsoft/worldpainter/icons/brick_edit.png"))); // NOI18N
    ...
    jLabel1.setText(
        "Define your custom object layer on this screen. Make sure all objects have a proper offset.");
    ...
    buttonAddFile.setIcon(
        new javax.swing.ImageIcon(
            getClass().getResource("/org/pepsoft/worldpainter/icons/brick_add.png"))); // NOI18N
    ...
    buttonRemoveFile.setIcon(
        new javax.swing.ImageIcon(
            getClass().getResource("/org/pepsoft/worldpainter/icons/brick_delete.png"))); // NOI18N
                ...
                layout
                    ...
                    .addGroup(
                        layout
                            ...
                            .addComponent(
                                buttonRemoveFile, javax.swing.GroupLayout.Alignment.TRAILING)
                            .addComponent(buttonEdit, javax.swing.GroupLayout.Alignment.TRAILING)
                            .addComponent(
                                buttonReloadAll, javax.swing.GroupLayout.Alignment.TRAILING)))
            .addGroup(
                layout
                    .createSequentialGroup()
                    .addGroup(
  ...
  private void buttonReloadAllActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_buttonReloadAllActionPerformed
    reloadObjects();
  } // GEN-LAST:event_buttonReloadAllActionPerformed

  private void buttonEditActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_buttonEditActionPerformed
    editObjects();
  } // GEN-LAST:event_buttonEditActionPerformed

  private void listObjectsMouseClicked(
      java.awt.event.MouseEvent evt) { // GEN-FIRST:event_listObjectsMouseClicked
    if (evt.getClickCount() == 2) {
      int row = listObjects.getSelectedIndex();
      if (row != -1) {
        WPObject object = listModel.getElementAt(row);
        EditObjectAttributes dialog =
            new EditObjectAttributes(SwingUtilities.getWindowAncestor(this), object, colourScheme);
        dialog.setVisible(true);
      }
    }
  } // GEN-LAST:event_listObjectsMouseClicked

  private void buttonAddFileActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_buttonAddFileActionPerformed
    addFilesOrDirectory();
  } // GEN-LAST:event_buttonAddFileActionPerformed

  private void buttonRemoveFileActionPerformed(
      java.awt.event.ActionEvent evt) { // GEN-FIRST:event_buttonRemoveFileActionPerformed
    removeFiles();
