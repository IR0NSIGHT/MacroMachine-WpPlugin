...
public class QuadTree {
  ...
  public static void main(String[] args) {
    ...
    List<Point2i> points =
        IntStream.range(0, 10000)
            .mapToObj(
                i ->
                    new Point2i(
                        (int) (Math.random() * 2000 - 1000), (int) (Math.random() * 2000 - 1000)))
            .toList();
    ...
    for (int i = 0; i < TILE_SIZE * TILE_SIZE; i++) {
      root.getClosest(
          new Point2i((int) (Math.random() * 2000 - 1000), (int) (Math.random() * 2000 - 1000)));
    }
  ...
  private boolean canSubDivide() {
    if (maxX - minX < MIN_SIZE || maxY - minY < MIN_SIZE) return false;
    return true;
  }
  ...
  /**
   * find tree node that fully encloses start -> end rect, and is the smallest treenode that does
   * so.
   *
   * @param start
   * @param end
  ...
  private Point2i getClosest(Point2i query, Point2i best, float bestDistSq) {
    ...
    for (QuadTree child : children) {
      ...
      if (childDistSq < bestDistSq) {
        ...
        if (best != null) bestDistSq = best.distanceSquared(query);
      }
    }
    return best;
  }
