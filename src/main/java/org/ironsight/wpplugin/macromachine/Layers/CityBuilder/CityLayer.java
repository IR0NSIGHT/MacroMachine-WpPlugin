...
public class CityLayer extends CustomLayer implements UndoListener {
  ...
  public void setDataAt(Dimension dimension, int blockX, int blockY, ObjectState state) {
    if (state == null || state.objectIndex < 0 || state.objectIndex >= objects.size()) return;

    database.setDataAt(
        blockX, blockY, getValueForState(state.rotation, state.mirrored, state.objectIndex));

    WPObject paintedSchem = getObjectForState(state);
    if (paintedSchem == null) return;
    Point3i dims = paintedSchem.getDimensions();
    for (int tileX = (blockX - dims.x) >> TILE_SIZE_BITS;
        tileX <= (blockX + dims.x) >> TILE_SIZE_BITS;
        tileX++) {
      for (int tileY = (blockY - dims.y) >> TILE_SIZE_BITS;
          tileY <= (blockY + dims.y) >> TILE_SIZE_BITS;
          tileY++) {
      ...
      for (int y = 0; y < object.getDimensions().y; y++) {
        ...
        for (int z = object.getDimensions().z - 1; z >= 0; z--) {
          ...
          if (mat != Material.AIR) {
            ...
            Color darkenFactor =
                new Color(
                    (int) (Math.max(0, Math.min(255, value * base.getRed()))),
  ...
  public void setObjectList(ArrayList<WPObject> newObjects) {
    ...
    for (int schematicIdx : oldIndicesToDelete)
      database.deleteAllWithValue(
          getValueForState(Direction.NORTH, false, schematicIdx), ID_BIT_MASK);
  ...
  public WPObject getObjectForState(ObjectState state) {
    ...
    if (objectIdx < 0 || objectIdx >= objects.size()) return null;
    WPObject object = objects.get(objectIdx);

    boolean mirror = state.mirrored;
    if (mirror) object = new MirroredObject(object, true, DefaultPlugin.JAVA_ANVIL_1_20_5);
  ...
  private WPObject getObjectForValue(int layerValue, Platform platform) {
    ...
    if (objectIdx == NO_DATA) return null;

    WPObject object = objects.get(objectIdx);

    boolean mirror = isMirrored(layerValue);
    if (mirror) object = new MirroredObject(object, true, platform);
  ...
  private int getObjectIdx(int layerValue) {
    if (layerValue == NO_DATA) return NO_DATA;
    int idBits = (layerValue & ID_BIT_MASK) >> ID_BIT_SHIFT;
    int index = idBits;
    if (objects.size() > index) return index;
    return NO_DATA;
  }

  private int getValueForState(Direction rotation, boolean isMirror, int schematicIdx) {
    return rotation.ordinal() << CityLayer.ROTATION_BIT_SHIFT
        | (isMirror ? 1 : 0) << CityLayer.MIRROR_BIT_SHIFT
  ...
  public LayerExporter getExporter(
      Dimension dimension, Platform platform, ExporterSettings settings) {
    return new CityLayerExporter(dimension, platform, settings, this);
  }

  public void repaintAll(Dimension dim) {
    if (dim == null) return;
    try {
