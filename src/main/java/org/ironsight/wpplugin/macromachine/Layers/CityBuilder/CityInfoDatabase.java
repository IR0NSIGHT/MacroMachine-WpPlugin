...
public class CityInfoDatabase implements Serializable {
  ...
  public boolean deleteAllWithValue(int index, int mask) {
    ...
    for (var tileInfo : tileInformation.values()) {
      ...
      for (Point p : toDelete) tileInfo.remove(p);
    }
    return deletedSome;
  }
  ...
  public int getDataAt(int blockX, int blockY) {
    var tile =
        tileInformation.getOrDefault(
            new Point(blockX >> TILE_SIZE_BITS, blockY >> TILE_SIZE_BITS), null);
    if (tile == null) return NO_DATA;
