...
public class ActionResource
{
        ...
        return ioProvider.setters.stream()
                ...
                .map(output -> {
                    return getNewEmptyAction(UUID.randomUUID()).withInput(new AlwaysIO())
                            ...
                            .withNewPoints(new MappingPoint[]{new MappingPoint(AlwaysIO.instance.getMinValue(),
                                    Math.round((output.getMaxValue() + output.getMinValue()) / 2f))}) // halfway point
    ...
    @GET
    @Path("/lastChange")
    @Operation(summary = "Get last modification timestamp", description = "Returns the timestamp of the most recent modification to the action container as milliseconds since the Unix epoch (equivalent to System.currentTimeMillis()).")
