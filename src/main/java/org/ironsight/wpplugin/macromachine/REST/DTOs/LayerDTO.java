...
public record LayerDTO(
    @Schema(description = "Display name of the layer", requiredMode = Schema.RequiredMode.REQUIRED)
        String name,
    @Schema(description = "Description of the layer", requiredMode = Schema.RequiredMode.REQUIRED)
        String description,
    @Schema(
            description = "Amount of possible values the layer can store",
            requiredMode = Schema.RequiredMode.REQUIRED)
        Layer.DataSize dataSize,
    @Schema(
            description = "IDK? Possible export application priority?",
            requiredMode = Schema.RequiredMode.REQUIRED)
        int priority,
    @Schema(description = "Unique, stable ID of layer", requiredMode = Schema.RequiredMode.REQUIRED)
        String id,
    @Schema(
            description = "Discrete. IDK about the semantic",
            requiredMode = Schema.RequiredMode.REQUIRED)
        boolean discrete,
    @Schema(
            description = "Specific layer category",
            requiredMode = Schema.RequiredMode.REQUIRED,
            examples = {"Custom Objects", "CityLayer", "Annotations"})
        String type,
