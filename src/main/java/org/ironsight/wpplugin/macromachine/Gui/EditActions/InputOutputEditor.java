...
public class InputOutputEditor extends LayerMappingPanel {
  public static void main(String[] args) {
    ...
    InputOutputEditor editor =
        new InputOutputEditor(
            f -> {
              System.out.println("update mapping to:" + f);
            });
    MappingAction lm =
        MappingAction.getNewEmptyAction().withInput(new PerlinNoiseIO(10, 20, 123456789, 3));
  ...
  private void onValueChanged(int[] newValues) {
    if (Arrays.equals(newValues, io.getEditableValues())) return;
    if (mapping == null) {
      System.err.println("mapping was null when trying to set values" + newValues);
      assert false;
      return;
    }
    MappingAction updated =
        isInput
