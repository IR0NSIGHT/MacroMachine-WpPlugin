...
public class RangeTableModel extends DefaultTableModel {
  ...
  public boolean isCellIllegalValue(int row, int column) {
    if (column == 0) return illegalStartRows.contains(row);
    else if (column == 1) return illegalEndRows.contains(row);
    return false;
  }
  ...
  public void addRows(int[] rows) {
    ...
    for (int i = rows.length - 1; i >= 0; i--) {
      ranges.add(
          rows[i],
          new ValueRange(
              new MappingPointValue(action.getInput().getMinValue(), action.getInput()),
  ...
  private ValueRange fromValues(double start, double end, int value) {
    return new ValueRange(
        new MappingPointValue((int) Math.round(start), action.getInput()),
  ...
  public void setAction(MappingAction action) {
    ...
    var newRanges =
        calculateRanges(action).stream()
            .map(
                startEnd ->
                    fromValues(startEnd.x, startEnd.y, action.map((int) Math.round(startEnd.y))))
  ...
  private List<Integer> getRowsWithIncorrectStarts(List<ValueRange> rows) {
    ...
    List<ValueRange> sortedRows =
        rows.stream().sorted(Comparator.comparing(r -> r.start().numericValue)).toList();
    for (int row = 1; row < sortedRows.size(); row++) {
      var thisRange = sortedRows.get(row);
      var previousRange = sortedRows.get(row - 1);
      if (thisRange.start.numericValue <= previousRange.end.numericValue) incorrectRows.add(row);
    }

    return incorrectRows;
  }

  private List<Integer> getRowsWithIncorrectEnd(
      List<ValueRange> rows, IPositionValueGetter getter) {
    ArrayList<Integer> incorrectRows = new ArrayList<>(rows.size());
  ...
  private ArrayList<ValueRange> validateRanges(
      List<ValueRange> ranges, IPositionValueGetter getter) {
    // Step 1 : sort by start value
    ArrayList<ValueRange> newRanges =
        new ArrayList<>(
        ...
        new ArrayList<>(
            newRanges.stream()
                .filter(
                    range ->
                        isLegalInput(
                            (IPositionValueGetter) range.start().mappingValue,
                            range.start().numericValue))
                .filter(
                    range ->
                        isLegalInput(
                            (IPositionValueGetter) range.end.mappingValue, range.end.numericValue))
                .filter(
                    range ->
                        isLegalOutput(
                            (IPositionValueSetter) range.value.mappingValue,
  ...
  private void updateValidationData() {
    HashSet<Integer> newIllegalEndRows =
        new HashSet<>(getRowsWithIncorrectEnd(ranges, action.getInput()));
  ...
  public MappingAction constructAction() {
    ...
    var missingIntervals =
        calculatedMissingIntervals(
            action.getInput().getMinValue(),
            action.getInput().getMaxValue(),
            validatedRanges,
            IPositionValueSetter.IGNORE_VALUE,
            action.getInput(),
            action.getOutput());
    ...
    var mappingPoints =
        validatedRanges.stream()
            .map(
                range ->
                    List.of(
                        new MappingPoint(range.start().numericValue, range.value().numericValue),
  ...
  private List<ValueRange> calculatedMissingIntervals(
      int min,
      int max,
      List<ValueRange> existingIntervals,
      int defaultValue,
      IPositionValueGetter getter,
      IPositionValueSetter setter) {

    List<ValueRange> result = new ArrayList<>();

    // sort intervals by start
    List<ValueRange> sorted =
        existingIntervals.stream()
    ...
    for (ValueRange vr : sorted) {
      ...
      if (start > current) {
        result.add(
            new ValueRange(
                new MappingPointValue(current, getter),
                new MappingPointValue(start - 1, getter),
    ...
    if (current <= max) {
      result.add(
          new ValueRange(
              new MappingPointValue(current, getter),
              new MappingPointValue(max, getter),
              new MappingPointValue(defaultValue, setter)));
    }
  ...
  private record ValueRange(
      MappingPointValue start, MappingPointValue end, MappingPointValue value) {}
}
