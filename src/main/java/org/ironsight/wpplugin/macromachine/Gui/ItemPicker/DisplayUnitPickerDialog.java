...
public class DisplayUnitPickerDialog extends JDialog {
   ...
   * @param filters
   */
  public DisplayUnitPickerDialog(
      ArrayList<IDisplayUnit> layerMappings,
  ...
  public static void main(String[] args) {
    ...
    PickerFilterOption showMacrosFilter =
    ...
    PickerFilterOption showActions =
        new PickerFilterOption("defaults", "show default actions") {
          final Set<UUID> matchingIds =
              defaultMappings.stream()
                  .map(MappingAction::getUid)
                  .collect(Collectors.toCollection(HashSet::new));

          @Override
          public boolean block(Object item) {
            return item instanceof MappingAction
                && matchingIds.contains(((MappingAction) item).getUid());
          }
        };

    PickerFilterOption showCustom =
        new PickerFilterOption("custom", "show user created actions") {
          final Set<UUID> matchingIds =
              container.queryAll().stream()
                  .map(MappingAction::getUid)
                  .collect(Collectors.toCollection(HashSet::new));
  ...
  private DefaultTableModel createTableModel(
      ArrayList<IDisplayUnit> items, Collection<IDisplayUnit> topActions) {
    int rowCount = items.size() + (topActions != null ? topActions.size() : 0);
    DefaultTableModel model =
        new DefaultTableModel(new Object[rowCount][1], new String[] {"Items"}) {
          @Override
  ...
  private void resizeRowHeights(JTable table) {
    ...
    for (int i = 0; i < table.getRowCount(); i++) table.setRowHeight(i, height);
  }
