...
public class ValuePreviewWindow extends MouseAdapter implements ActionListener {
  ...
  void setMouseOver(Point p) {
    ...
    if (row >= 0) {
      ...
      if (!(cell instanceof MappingPointValue)) return;
      ...
      previewLabel.setIcon(
          new ImageIcon(
              scaledImage.getScaledInstance(
                  previewWindow.getWidth(), previewWindow.getHeight(), SCALE_FAST)));

      // Position the window near the mouse
      Point locationOnScreen = table.getLocationOnScreen();
      previewWindow.setLocation(
          locationOnScreen.x + (int) p.getX() + 15, locationOnScreen.y + (int) p.getY() + 15);
  ...
  public void actionPerformed(ActionEvent e) {
    if (e.getSource() instanceof JCheckBox cb) allowWindow = cb.isSelected();
    else assert false : "you re using it wrong";
  }
}
