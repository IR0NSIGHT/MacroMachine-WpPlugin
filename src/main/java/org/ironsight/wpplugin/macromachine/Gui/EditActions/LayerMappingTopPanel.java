...
public class LayerMappingTopPanel extends LayerMappingPanel {
  ...
  private void updateFromInputs() {
    if (!isInit && isAllowEvents() && mapping != null) {
      ...
      if (!inputSelect
          .getSelectedProvider()
          .getName()
          .equals(mapping.input.getName())) // rebuild map
      newMap =
            newMap
                .withInput((IPositionValueGetter) inputSelect.getSelectedProvider())
                .withNewPoints(mapping.getMappingPoints());
      if (!outputSelect
          .getSelectedProvider()
          .getName()
          .equals(mapping.output.getName())) { // rebuild map
        ...
        if (output.getProviderType()
            == ProviderType.INTERMEDIATE_SELECTION) // special case: for filters, all
        ...
        newMap = newMap.withOutput(output).withNewPoints(mps);
      }
  ...
  protected void initComponents() {
    ...
    actionTypeComboBox.addActionListener(
        e -> {
