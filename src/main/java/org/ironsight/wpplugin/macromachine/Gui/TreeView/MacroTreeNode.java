...
public class MacroTreeNode implements TreeNode {
  ...
  public MacroTreeNode(MappingActionContainer actions, MacroContainer macros) {
    ...
    for (MacroTreeNode child : children) child.setParent(this);
  ...
  public MacroTreeNode(
      Macro macro, boolean isActive, MappingActionContainer actions, MacroContainer macros) {
    payload = macro;
    ...
    for (UUID id : macro.getExecutionUUIDs()) {
      if (macros.queryContains(id))
        nodes.add(
            new MacroTreeNode(
                macros.queryById(id), macro.getActiveActions()[idx], actions, macros));
  ...
  public boolean equals(Object o) {
    ...
    if (this.getPayloadType() != that.getPayloadType()) return false;
