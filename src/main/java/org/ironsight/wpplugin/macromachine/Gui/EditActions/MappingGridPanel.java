...
public class MappingGridPanel extends LayerMappingPanel
  ...
  protected void initComponents() {
    ...
    cursorCheckbox.addActionListener(
        e -> {
  ...
  protected void paintCursor(Graphics g) {
    ...
    ((Graphics2D) g)
        .setStroke(
            new BasicStroke(
                2, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 0, new float[] {4, 10}, 0));
    // find closest grid pos to mouse
    Point gridPos = pixelToGrid(mousePosX, mousePosY);
    if (mapping.sanitizeInput(gridPos.x) != gridPos.x
        || mapping.sanitizeOutput(gridPos.y) != gridPos.y) return;
    paintLineInGrid(
        gridPos.x, mapping.output.getMinValue(), gridPos.x, mapping.output.getMaxValue(), g);
    paintLineInGrid(
        mapping.input.getMinValue(), gridPos.y, mapping.input.getMaxValue(), gridPos.y, g);
  ...
  protected void paintCurveLines(Graphics2D g) {
    g.setStroke(
        new BasicStroke(
            3, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 0, new float[] {4, 4}, 0));
    g.setColor(Color.red);
    if (mapping.output.isDiscrete()) {
      ...
      for (int i = 1; i < mapping.getMappingPoints().length; i++) {
        ...
        if (!mapping.getOutput().isIgnoreValue(a1.output)
            && !mapping.getOutput().isIgnoreValue(a2.output))
      ...
      for (int i = 0; i < mapping.getMappingPoints().length - 1; i++) {
        ...
        if (!mapping.getOutput().isIgnoreValue(a.output)
            && !mapping.getOutput().isIgnoreValue(b.output))
  ...
  private void paintGrid(Graphics g) {
    ...
    this.GRID_X_SCALE =
        pixelSizeX / ((float) mapping.input.getMaxValue() - mapping.input.getMinValue());
    this.GRID_Y_SCALE =
        pixelSizeY / ((float) mapping.output.getMaxValue() - mapping.output.getMinValue());
    ...
    {
      g2d.setFont(
          new Font(g2d.getFont().getName(), g2d.getFont().getStyle(), g2d.getFont().getSize() * 3));
      int fontheight = g2d.getFontMetrics().getHeight();

      Point center =
          gridToPixel(
              (mapping.input.getMaxValue() + mapping.input.getMinValue()) / 2f,
  ...
  private void paintMappingPoints(Graphics2D g2d) {
    ...
    for (MappingPoint p : mapping.getMappingPoints()) {
      if (mapping.getOutput().isIgnoreValue(p.output)) continue;
  ...
  private Point pixelToGrid(int pixelX, int pixelY) {
    ...
    int gridYPressed =
        Math.round((pixelSizeY - (pixelY - shiftY)) / GRID_Y_SCALE)
  ...
  private boolean selectPointNear(int pixelX, int pixelY, int maxDist) {
    ...
    final Function<MappingPoint, Double> distance =
        p -> {
  ...
  private Point gridToPixel(float gridX, float gridY) {
    ...
    int pixelY =
        Math.round(pixelSizeY - ((gridY - mapping.output.getMinValue()) * GRID_Y_SCALE)) + shiftY;
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    if (e.getButton() == MouseEvent.BUTTON1) {
      if (!dragged) { // INSERT POINT
        ...
        if (!hit) { // selected point didnt change,
          ...
          MappingPoint[] newPoints =
              Arrays.copyOf(mapping.getMappingPoints(), mapping.getMappingPoints().length + 1);
          MappingPoint p =
      ...
      if (distPixel > pointHitBoxRadius) return; // abort, click is to far away from selction
      ...
      // control point
      MappingPoint[] newPoints =
          Arrays.stream(mapping.getMappingPoints())
  ...
  public void mouseDragged(MouseEvent e) {
    ...
    if (drag) {
      ...
      if (gridX == draggedPoint.input && gridY == draggedPoint.output) return; // nothing to do.
      ...
      for (MappingPoint mp : newSelection) setInputSelection(mp, true);
    }
  }
