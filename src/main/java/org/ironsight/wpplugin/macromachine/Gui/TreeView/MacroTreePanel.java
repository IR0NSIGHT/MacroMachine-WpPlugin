...
public class MacroTreePanel extends JPanel {
  ...
  public MacroTreePanel(
      MacroContainer container,
      MappingActionContainer mappingContainer,
  ...
  public static boolean isValidItem(IDisplayUnit item) {
    if (item instanceof Macro) {
      ...
      return isValidItem(((MappingAction) item).getInput())
          && isValidItem(((MappingAction) item).getOutput());
  ...
  private static Macro cloneMacro(
      Macro original,
      ArrayList<MappingAction> clonedActions,
    ...
    for (UUID childUID : original.getExecutionUUIDs()) {
      if (mappingContainer.queryContains(childUID)) {
        MappingAction actionClone =
            mappingContainer.queryById(childUID).withUUID(UUID.randomUUID());
  ...
  private boolean verifyTreePathExists(
      MacroTreeNode node, Object[] path, int index, Object[] newPath) {
    newPath[index] = node;
    if (index >= path.length - 1) return node.equals(path[index]);
    else {
  ...
  private List<TreePath> findExpandedPaths(
      JTree tree, MacroTreeNode node, LinkedList<TreePath> expanded) {
    TreePath path = node.getPath();
    if (tree.isExpanded(path)) expanded.add(path);
    for (MacroTreeNode child : node.getChildren()) {
  ...
  private List<TreePath> findAllPaths(
      JTree tree, MacroTreeNode node, LinkedList<TreePath> allPaths) {
    TreePath path = node.getPath();
    allPaths.add(path);
    for (MacroTreeNode child : node.getChildren()) {
  ...
  private TreePath findPathEquivalent(
      Object[] oldPath, MacroTreeNode newNode, int index, Object[] newPath) {
    newPath[index] = newNode;
    if (index >= oldPath.length - 1) return new TreePath(newPath);
    else {
  ...
  private void update(Set<UUID> selectItems) {
    ...
    if (!selectItems.isEmpty()) {
      ...
      for (TreePath oldPath : expanded) {
        TreePath newPath =
            findPathEquivalent(oldPath.getPath(), newRoot, 0, new Object[oldPath.getPath().length]);
        if (newPath != null) newExpanded.add(newPath);
      }
    }
  ...
  ArrayList<UUID> getSelectedUUIDs(boolean macros, boolean actions) {
    ...
    if (tree.getSelectionPaths() == null) return selectedUUIDs;
    ...
    Arrays.sort(
        ...
        Comparator.comparingInt(
            path ->
                Arrays.asList(tree.getPathForRow(0).getPath())
  ...
  public void selectItemInTree(SaveableAction macroOrAction) {
    ...
    if (currentSelPath != null
        && currentSelPath.getLastPathComponent() instanceof MacroTreeNode node
        && node.getPayloadType() == GlobalActionPanel.SELECTION_TPYE.MACRO) {
  ...
  private void onSearchEnter(String searchString) {
    ...
    List<TreePath> allPaths =
        findAllPaths(tree, (MacroTreeNode) tree.getModel().getRoot(), new LinkedList<>());
    List<TreePath> matchingPaths =
        allPaths.stream()
            .filter(
                path -> {
  ...
  private void onDeleteItem(ActionEvent e) {
    ...
    for (Macro m : macroContainer.queryAll()) {
      if (deletedUUIDS.contains(m.getUid())) continue;
      Macro updated =
          m.withUUIDs(
              Arrays.stream(m.executionUUIDs)
                  .filter(a -> !deletedUUIDS.contains(a))
                  .toArray(UUID[]::new));
      updatedMacros.add(updated);
    }

    macroContainer.updateMapping(
        GlobalActionPanel::ErrorPopUpString, updatedMacros.toArray(new Macro[0]));
  ...
  private void init() {
    ...
    JButton helpButton =
        getHelpButton(
            ...
            "This view shows your global list of macros and actions. You can"
                ...
                + " your "
                + "save-files. These are global and the same for all projects.\n"
                + " Press 'Apply'"
                + " to "
                + "apply a macro as a global operation to "
                + "your map.");
    ...
    { // right click button list
      ...
      deleteButton.setToolTipText(
          "Delete all selected macros permanently. Nested macros are not deleted.");
  ...
  private void onCloneMacroPressed() {
    ...
    mappingContainer.updateMapping(
        GlobalActionPanel::ErrorPopUpString, clonedActions.toArray(MappingAction[]::new));
    macroContainer.updateMapping(
        GlobalActionPanel::ErrorPopUpString, clonedMacros.toArray(Macro[]::new));

    SwingUtilities.invokeLater(
        () -> {
          HashSet<UUID> newMacroUUIDs =
              new HashSet<>(clonedMacros.stream().map(Macro::getUid).toList());
  ...
  private void onAddMacroPressed(boolean useSelectedAsChildren) {
    ...
    if (useSelectedAsChildren) {
      ...
      for (UUID uuid : actionUids) {
        MappingAction clone =
            mappingContainer.addMapping().withValuesFrom(mappingContainer.queryById(uuid));
  ...
  private void onExportMacroPressed() {
    ...
    fileChooser.setCurrentDirectory(
        new File(MacroMachineWindow.getDialog().getLastDirectoryPicked()));
    fileChooser.setFileSelectionMode(
        JFileChooser.DIRECTORIES_ONLY); // Only allow directory selection
  ...
  private void onImportMacroPressed() {
    ...
    MacroMachineWindow.getDialog()
        .setLastDirectoryPicked(fileChooser.getCurrentDirectory().getPath());
    if (result == JFileChooser.APPROVE_OPTION) {
      for (File selected : fileChooser.getSelectedFiles()) {
        ContainerIO.importFile(
            ...
            MacroContainer.getInstance(),
            selected,
            new ConflictResolveImportPolicy(
                MacroContainer.getInstance(),
                MappingActionContainer.getInstance(),
  ...
  private void onApply(boolean isDebug) {
    ...
    if (macroIsCurrentlyExecuting) return;
    ...
    if (selected == null
        || selected.getLastPathComponent() == null
    ...
    final Macro executingMacro =
        macroContainer.queryById(
            ((MacroTreeNode) selected.getLastPathComponent()).getMacro().getUid());
    if (executingMacro == null) {
      macroIsCurrentlyExecuting = false;
      return;
    }
    // get macros
    // FIXME move getTreeStepper into constructor of callback? instead of wierd
    // pingpong spgehtti
    macroExecutionCallback = panel.getUserCallback();

    treeStepper =
        new TreeDebugStepperUI(
            (MacroTreeNode) selected.getLastPathComponent(),
            macroContainer,
