...
public class IoSelectLabel extends JPanel {
  ...
  /**
   * component to display currently selected input or output (getter/setter) and select-editor on
   * click
   *
  ...
  private void showPickingDialog() {
    new DisplayUnitPickerDialog(
            new ArrayList<IDisplayUnit>(provider.getItems()),
            this::onPickerSubmit,
  ...
  private void onPickerSubmit(IDisplayUnit selected) {
    ...
    onChangeCallback.accept(
        ((IMappingValue) selected)
            .instantiateFrom(((IMappingValue) selected).getSaveData())); // return
    // clone
  }
