...
public class ProviderTest {
  ...
  void AllProvidersOutOfRangeTest() {
    ...
    for (ProviderType type : ProviderType.values()) {
      ...
      assertFalse(
          dim.getExtent().contains(testPosX >> TILE_SIZE_BITS, testPosY >> TILE_SIZE_BITS),
  ...
  void AllProvidersSetValue() {
    /**
     * tests takes every IO type, creates a dimension and the sets + gets every allowed value for
     * that IO type
     */
    Dimension dim = createDimension(new Rectangle(0, 0, TILE_SIZE, TILE_SIZE), 62);
    ...
    for (ProviderType type : ProviderType.values()) {
      ...
      for (int value : ioInstance.getAllPossibleValues()) {
        if (ioInstance instanceof IPositionValueSetter setter) {
          if (setter.isIgnoreValue(value)) continue;
          ...
          if (ioInstance instanceof IPositionValueGetter) {
            ...
            assertEquals(
                value,
                readValue,
                "set pos to have value="
                    + value
                    + " but afterwards read value="
                    + readValue
                    + ioInstance.getName());
          }
        }
  ...
  void WaterDepthProviderSetGetValue() {
    ...
    assertEquals(
        77 - 3,
        dim.getHeightAt(17, 18),
        "water depth should change terrain height to achieve waterdepth");
  }
  ...
  void BitLayerSpraypaintProviderGetSetValue() {
    ...
    for (int y = 0; y < 100; y++) {
      for (int x = 0; x < 100; x++) {
        ...
        if (dim.getBitLayerValueAt(Frost.INSTANCE, x, y)) hits++;
      }
    }
    assertTrue(
        100 * spraypaintChancePercent * 0.99 <= hits, "hits less often than 99% requested chance");
    assertTrue(
        100 * spraypaintChancePercent * 1.01 >= hits, "hits more often than 101% requested chance");
    ...
    for (int i = 0; i < 100; i++) {
      ...
      assertEquals(
          value,
          dim.getLayerValueAt(Frost.INSTANCE, 17, 18) == 0,
  ...
  void SelectionTest() {
    ...
    assertEquals(
        true,
        dim.getBitLayerValueAt(SelectionBlock.INSTANCE, 18, 19),
        "io wrties to block select layer");
    ...
    assertEquals(
        false,
        dim.getBitLayerValueAt(SelectionChunk.INSTANCE, 18, 19),
        "io sets block and chunk select to " + "zero");
  }
  ...
  void TestVanillaBiomeIO() {
    ...
    assertEquals(
        Minecraft1_21Biomes.BIOME_BIRCH_FOREST, dim.getLayerValueAt(Biome.INSTANCE, 19, 20));
  }
  ...
  void TestRandomIO() {
    ...
    for (int x = -1000; x < 1300; x++) {
      for (int y = -300; y < 2700; y++) {
        int value = input.getValueAt(null, x, y);
        if (value == RandomNoise.PASS) passCount++;
        else if (value == RandomNoise.BLOCK) rejectCount++;
        else {
