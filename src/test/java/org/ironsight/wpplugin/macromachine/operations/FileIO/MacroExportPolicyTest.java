...
class MacroExportPolicyTest {
  ...
  void testEmptyMacroExportPolicy() {
    ...
    for (Macro m : macroContainer.queryAll()) {
      if (m.getUid().equals(emptyMacroUID)) assertTrue(macroExportPolicy.allowImportExport(m));
      else assertFalse(macroExportPolicy.allowImportExport(m));
    }
  ...
  void testSimpleMacroExportPolicy() {
    ...
    for (Macro m : macroContainer.queryAll()) {
      if (m.getUid().equals(simpleMacroUID)) assertTrue(macroExportPolicy.allowImportExport(m));
      else assertFalse(macroExportPolicy.allowImportExport(m));
    }
    for (MappingAction a : actionContainer.queryAll())
      if (a.getUid().equals(applyGrassAction01) || a.getUid().equals(applyGrassAction02))
        assertTrue(macroExportPolicy.allowImportExport(a));
      else assertFalse(macroExportPolicy.allowImportExport(a));
  }
  ...
  void testComplexMacroWithNestingExportPolicy() {
    ...
    for (Macro m : macroContainer.queryAll()) {
      if (m.getUid().equals(complexMacroUID)
          || m.getUid().equals(simpleMacroUID)
          || m.getUid().equals(emptyMacroUID)) assertTrue(macroExportPolicy.allowImportExport(m));
      else assertFalse(macroExportPolicy.allowImportExport(m));
    }
