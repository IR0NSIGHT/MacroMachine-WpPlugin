...
class FilterUtilsTest {
    ...
    void asOutsideRangeFilter() {
        MappingAction filter = new MappingAction(
                new NibbleLayerSetter(DeciduousForest.INSTANCE, false),
                new ActionFilterIO(),
                new MappingPoint[]{new MappingPoint(5, IGNORE_VALUE)},
                ActionType.SET,
                "my test range filter",
                "descr",
                UUID.randomUUID()
        );
        {
        ...
        }
        {
            ...
            var passingOutputs = Arrays.stream(rangeFilter.getInput().getAllInputValues()).map(rangeFilter::map).mapToObj(o -> o != BLOCK_VALUE).toArray();
            // array shows which index of input will pass (true) or be filtered out (false)
            assertArrayEquals(new Boolean[]{ true,true,true,true, false,false,false,false,false,false, true,true,true,true,true, true
            }, passingOutputs, "outside range filter failed");
        }
        {
            var rangeFilter = FilterUtils.asRangeFilter(filter, 0, 9, false);
            var passingOutputs = Arrays.stream(rangeFilter.getInput().getAllInputValues()).map(rangeFilter::map).mapToObj(o -> o != BLOCK_VALUE).toArray();
            assertArrayEquals(new Boolean[]{ false,false,false,false, false,false,false,false,false,false, true,true,true,true,true, true
            }, passingOutputs, "outside range filter failed");
        }
        {
            var rangeFilter = FilterUtils.asRangeFilter(filter, 9, 15, false);
            var passingOutputs = Arrays.stream(rangeFilter.getInput().getAllInputValues()).map(rangeFilter::map).mapToObj(o -> o != BLOCK_VALUE).toArray();
            assertArrayEquals(new Boolean[]{ true,true,true,true,true, true, true,true,true, false,false,false,false, false,false, false
            }, passingOutputs, "outside range filter failed");
        }
        {
            var rangeFilter = FilterUtils.asRangeFilter(filter, 0, 15, false);
            var passingOutputs = Arrays.stream(rangeFilter.getInput().getAllInputValues()).map(rangeFilter::map).mapToObj(o -> o != BLOCK_VALUE).toArray();
            assertArrayEquals(new Boolean[]{ false,false,false,false, false,false, false, false,false, false,false,false,false, false,false, false
            }, passingOutputs, "outside range filter failed");
        }
    }
    @Test
    void asInsideRangeFilter() {
        ...
        }
        {
            ...
            var passingOutputs = Arrays.stream(rangeFilter.getInput().getAllInputValues()).map(rangeFilter::map).mapToObj(o -> o != BLOCK_VALUE).toArray();
            // array shows which index of input will pass (true) or be filtered out (false)
            assertArrayEquals(new Boolean[]{  false,false,false,false, true,true,true,true,true, true,  false,false,false,false,false,false
            }, passingOutputs, "inside range filter failed");
        }
        {
            var rangeFilter = FilterUtils.asRangeFilter(filter, 0, 9, true);
            var passingOutputs = Arrays.stream(rangeFilter.getInput().getAllInputValues()).map(rangeFilter::map).mapToObj(o -> o != BLOCK_VALUE).toArray();
            assertArrayEquals(new Boolean[]{ true,true,true,true, true,true,true,true,true,true, false,false,false,false,false, false
            }, passingOutputs, "inside range filter failed");
        }
        {
            var rangeFilter = FilterUtils.asRangeFilter(filter, 9, 15, true);
            var passingOutputs = Arrays.stream(rangeFilter.getInput().getAllInputValues()).map(rangeFilter::map).mapToObj(o -> o != BLOCK_VALUE).toArray();
            assertArrayEquals(new Boolean[]{ false,false,false,false,false, false, false,false,false, true,true,true,true, true,true, true
            }, passingOutputs, "inside range filter failed");
        }
        {
            var rangeFilter = FilterUtils.asRangeFilter(filter, 0, 15, true);
            var passingOutputs = Arrays.stream(rangeFilter.getInput().getAllInputValues()).map(rangeFilter::map).mapToObj(o -> o != BLOCK_VALUE).toArray();
            assertArrayEquals(new Boolean[]{ true,true,true,true, true,true, true, true,true, true,true,true,true, true,true, true
            }, passingOutputs, "inside range filter failed");
        }
    }
}