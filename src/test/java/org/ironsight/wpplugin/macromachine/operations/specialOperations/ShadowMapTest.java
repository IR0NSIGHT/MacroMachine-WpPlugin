...
class ShadowMapTest {
  ...
  void expandBinaryLinear() {
    ...
    { // right to left after left to right
      int[] row =
          ShadowMap.expandBinaryLinear(
              new int[] {100, 100, 100, 0, 100, 0, 0, 100, 100, 100, 100}, 1, 0, 1);
  ...
  void expandBinaryMask() {
    ...
    assertEquals(
        dist(p2x, p2y, 11, 29),
        container.getValueAt(11, 29),
        "p2 is the closest point to this position");
    ...
    assertEquals(
        31, container.getValueAt(9, 37), "its closer to p2, so should take this distance to p1");
  }
  ...
  void expandBinaryLinearColumn() {
    { // takes existing value, keeps it and writes distance map into the first16bits
      int[] horizDist =
          new int[] {0xFFFF, 0xFFFF, 7, 0xFFFF, 0xFFFF, 0xFFFF, 3, 0xFFFF, 0xFFFF, 0xFFFF};
