...
class MacroConcurrentApplicatorTest {
  ...
  public void testRunDeserializedMacro() throws IOException, InterruptedException {
    ...
    Consumer<UUID> callbackAfterRun =
        (uuid1) -> {
          counter[0]++;
        };
    var applicator =
        new MacroConcurrentApplicator(macroContainer, actionContainer, () -> dim, callbackAfterRun);
  ...
  public void testRunningMacro() throws InterruptedException {
    ...
    var action =
        new MappingAction(
            ...
            uuid);
    var macro =
        new Macro("test macro", "desc", new UUID[] {uuid}, UUID.randomUUID(), new boolean[] {true});
