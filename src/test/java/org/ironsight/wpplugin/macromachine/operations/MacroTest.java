...
class MacroTest {
  ...
  void withReplacedUUIDs() {
    Macro initial =
        new Macro(
            "Test",
            "descr",
            new UUID[] {UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID()},
            UUID.randomUUID(),
  ...
  void applyMacroToDimension() {
    ...
    MappingAction setCyanAction =
        new MappingAction(
            new TerrainHeightIO(0, 255),
    ...
    Macro m =
        new Macro(
            "paint cyan",
            "test",
            new UUID[] {setCyanAction.getUid()},
            UUID.randomUUID(),
            new boolean[] {true});
    macroContainer.addMapping(m);

    assertEquals(
        AnnotationSetter.ANNOTATION_ABSENT, dim.getLayerValueAt(Annotations.INSTANCE, 17, -89));
    assertEquals(
        AnnotationSetter.ANNOTATION_ABSENT, dim.getLayerValueAt(Annotations.INSTANCE, 0, 0));
    assertEquals(
        AnnotationSetter.ANNOTATION_ABSENT, dim.getLayerValueAt(Annotations.INSTANCE, -256, -256));
    assertEquals(
        AnnotationSetter.ANNOTATION_ABSENT, dim.getLayerValueAt(Annotations.INSTANCE, 512, 512));
    ...
    assertEquals(
        AnnotationSetter.ANNOTATION_CYAN, dim.getLayerValueAt(Annotations.INSTANCE, 17, -89));
    assertEquals(AnnotationSetter.ANNOTATION_CYAN, dim.getLayerValueAt(Annotations.INSTANCE, 0, 0));
    assertEquals(
        AnnotationSetter.ANNOTATION_CYAN, dim.getLayerValueAt(Annotations.INSTANCE, -256, -256));
    assertTrue(dim.isTilePresent(511 >> TILE_SIZE_BITS, 511 >> TILE_SIZE_BITS));
    assertEquals(
        AnnotationSetter.ANNOTATION_CYAN, dim.getLayerValueAt(Annotations.INSTANCE, 511, 511));
  }
}
