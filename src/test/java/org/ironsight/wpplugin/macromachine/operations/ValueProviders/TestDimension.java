...
public class TestDimension {
  ...
  public static Dimension createDimension(DimensionParams params) {
    ...
    final Dimension dimension =
        new Dimension(world, "Surface", params.seed, tileFactory, NORMAL_DETAIL);
    {
  ...
  private static TileFactory createTileFactory(
      int terrainHeight, int minHeight, int maxHeight, int waterLevel, int seed, Terrain terrain) {
    SimpleTheme theme = SimpleTheme.createSingleTerrain(terrain, minHeight, maxHeight, waterLevel);
    return new HeightMapTileFactory(
        seed, new ConstantHeightMap(terrainHeight), minHeight, maxHeight, false, theme);
  }

  /**
   * assert the dimension is created with the values the user has defined, such as terrainheight,
   * waterlevel etc
   */
  @Test
