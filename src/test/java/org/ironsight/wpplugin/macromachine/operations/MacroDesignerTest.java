...
public class MacroDesignerTest {
  ...
  void insertSingleActionIntoEmptyMacro() {
    ...
    assertNotEquals(
        inputItem.getUid(),
        inserted.getExecutionUUIDs()[0],
        "macro receives a clone, not the original " + "input");
    MappingAction addedItem = container.queryById(inserted.getExecutionUUIDs()[0]);
    assertTrue(
        addedItem.equalIgnoreUUID(inputItem),
        "input and added item have same values, but not UUID");
    assertNotEquals(
        addedItem.getUid(),
        inputItem.getUid(),
        "input and added item are not true equal (different uuid)");
    assertEquals(
        1, container.queryAll().size(), "inserting cloned the action and updated the container");
  ...
  void insertSingleActionIntoNonEmptyMacroMiddleSelection() {
    ...
    Macro inserted =
        Macro.insertSaveableActionToList(
            ...
            container::addMapping,
            action -> container.updateMapping(action, Assertions::fail),
            new int[] {2}, // insert after index=2 in
            // list => at index 3
            newSelection);
    assertEquals(5, inserted.getExecutionUUIDs().length);
    UUID[] expectedIds = Arrays.copyOf(actionIds, 5);
  ...
  void insertManyActionsIntoMacro() {
    ...
    Macro inserted =
        Macro.insertSaveableActionToList(
            ...
            action -> container.updateMapping(action, Assertions::fail),
            new int[] {2, 3}, // insert after index=2 in list
            // => at index 3
            newSelection);
  ...
  void insertNestedMacroIntoMacro() {
    ...
    Macro inputItem =
        new Macro("myNewMacro", "descr", new UUID[0], UUID.randomUUID(), new boolean[0]);
