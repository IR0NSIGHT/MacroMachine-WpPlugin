...
class MappingActionValueTableModelTest {
  ...
  void rebuildDataWithAction() {
    ...
    MappingAction action =
        MappingAction.getNewEmptyAction()
            ...
            .withNewPoints(
                new MappingPoint[] {
                  new MappingPoint(1, 2), new MappingPoint(5, 6), new MappingPoint(9, -1)
                })
  ...
  void isCellEditable() {
    ...
    model.rebuildModelFromAction(
        action.withNewPoints(new MappingPoint[] {new MappingPoint(17, 18)}));
  ...
  void setValueAt() {
    ...
    assertEquals(
        new MappingPointValue(9, action.getOutput()),
        model.getValueAt(3, 1),
        "value was inserted at index");
    assertEquals(
        action.withNewPoints(new MappingPoint[] {new MappingPoint(3, 9)}),
        model.constructMapping());
  }
  ...
  void setValuesAt() {
    ...
    // point1
    assertEquals(
        new MappingPointValue(point1.input, action.getInput()), model.getValueAt(point1.input, 0));
    assertEquals(
        new MappingPointValue(point1.output, action.getOutput()),
        model.getValueAt(point1.input, 1));

    // point2
    assertEquals(
        new MappingPointValue(point2.input, action.getInput()), model.getValueAt(point2.input, 0));
    assertEquals(
        new MappingPointValue(point2.output, action.getOutput()),
        model.getValueAt(point2.input, 1));

    // point3
    assertEquals(
        new MappingPointValue(point3.input, action.getInput()), model.getValueAt(point3.input, 0));
    assertEquals(
        new MappingPointValue(point3.output, action.getOutput()),
        model.getValueAt(point3.input, 1));

    // single point change: modify output of point1

    model.setValuesAt(
        new MappingPointValue(point1.output + 5, action.getInput()), new int[] {point1.input}, 1);
    assertEquals(
        new MappingPointValue(point1.input, action.getInput()), model.getValueAt(point1.input, 0));
    assertEquals(
        new MappingPointValue(point1.output + 5, action.getOutput()),
        model.getValueAt(point1.input, 1));
    ...
    assertEquals(
        new MappingPointValue(point1.output, action.getOutput()), model.getValueAt(row, 1));

    // change many control points at once, output modified
    model.setValuesAt(
        new MappingPointValue(point1.output + 5, action.getInput()),
        new int[] {point1.input, point2.input, point3.input},
        1);
    assertEquals(
        new MappingPointValue(point1.input, action.getInput()), model.getValueAt(point1.input, 0));
    assertEquals(
    ...
    for (int rowIdx = 0; rowIdx < allIndices.length; rowIdx++) {
      if (rowIdx == point1.input || rowIdx == point2.input || rowIdx == point3.input) continue;
