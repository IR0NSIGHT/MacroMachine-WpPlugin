...
class ContinuousCurveTest {
    ...
    void continuousCurve() {
        ...
        p = p.setHandleByIdx(RiverHandleInformation.setValue(p.handleByIndex(0), RiverHandleInformation.RiverInformation.RIVER_RADIUS, startWidthRiver), 0);
        Assertions.assertEquals(startWidthRiver, RiverHandleInformation.getValue(p.handleByIndex(0), RiverHandleInformation.RiverInformation.RIVER_RADIUS));

        p = p.setHandleByIdx(RiverHandleInformation.setValue(p.handleByIndex(1), RiverHandleInformation.RiverInformation.RIVER_RADIUS, startWidthRiver), 1);
        Assertions.assertEquals(startWidthRiver, RiverHandleInformation.getValue(p.handleByIndex(1), RiverHandleInformation.RiverInformation.RIVER_RADIUS));


        //set river final width values
        p = p.setHandleByIdx(RiverHandleInformation.setValue(p.handleByIndex(p.amountHandles() - 1), RiverHandleInformation.RiverInformation.RIVER_RADIUS, 17), p.amountHandles() - 1);
        Assertions.assertEquals(17, RiverHandleInformation.getValue(p.handleByIndex(p.amountHandles() - 1), RiverHandleInformation.RiverInformation.RIVER_RADIUS));
        p = p.setHandleByIdx(RiverHandleInformation.setValue(p.handleByIndex(p.amountHandles() - 2), RiverHandleInformation.RiverInformation.RIVER_RADIUS, 17), p.amountHandles() - 2);
        Assertions.assertEquals(17, RiverHandleInformation.getValue(p.handleByIndex(p.amountHandles() - 2), RiverHandleInformation.RiverInformation.RIVER_RADIUS));

        //set river radius to inherit on all but first two and last two values
        for (int i = 2; i < p.amountHandles() - 2; i++) {
            p = p.setHandleByIdx(RiverHandleInformation.setValue(p.handleByIndex(i), RiverHandleInformation.RiverInformation.RIVER_RADIUS, RiverHandleInformation.INHERIT_VALUE), i);
            Assertions.assertEquals(RiverHandleInformation.INHERIT_VALUE, RiverHandleInformation.getValue(p.handleByIndex(i), RiverHandleInformation.RiverInformation.RIVER_RADIUS));
        }
...

        {
            ...
            //allow only setting one or more values and propagate to the other handles
            {
                ...
                for (int i = 0; i < curve.curveLength(); i++) {
                    ...
                    assertEquals(30, curve.getInfo(RiverHandleInformation.RiverInformation.TRANSITION_RADIUS, i), 0.01f);
                }
            }
