...
function generateMappings(
    ...
    if (output.discrete) {
        ...
        for (const i of inputValues(input)) {
            ...
            const mappingPointIdx = Math.max(
                Math.min(j + 1, mappingPoints.length - 1),
                0
            );
...
function generateInterpolatedOutput(
    ...
    const mappings = new Array(getter.values.length).fill(
        setter.min
    );
    ...
    for (let i = 0; i < points.length - 1; i++) {
        ...
        if (
            isIgnoreValue(setter, low.y) ||
            isIgnoreValue(setter, high.y)
        ) {
            ignore = low.y;
        ...
        for (let inputValue = low.x + 1; inputValue <= high.x; inputValue++) {
            ...
            const outputValue = useIgnoreOutput
                ? ignore
                : Math.round((1 - t) * low.y + t * high.y);
...
export const fetchActionWithPoints = async (actionUuid: string, newPoints: MappingPoint[]): Promise<MMAction> => {
