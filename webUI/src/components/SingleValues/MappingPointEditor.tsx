  ...
  return (
    <Dialog open={props.editorActive} onClose={props.onClose} maxWidth="sm" fullWidth>
      ...
      >
        ...
        <div>
          for all points where {draft.input.displayName}=
          {draft.input.values.find((v) => v.numericValue === draft.x)?.displayName ?? draft.x},{" "}
          {draft.output.displayName} is {typeConjugated}{" "}
          {draft.output.values.find((v) => v.numericValue === draft.y)?.displayName ?? draft.y}
        </div>
      </DialogContent>
