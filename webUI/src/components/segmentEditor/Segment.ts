...
export const splitAt = (segments: Segments, position: number): Segments => {
  ...
  if (segmentToBeSplit.end - segmentToBeSplit.start <= 2) {
    // smallest split is 0-1 and 1-2, so segment has to be at least 3 wide to be split
    return segments; // don't split if segment is too small to be split
  }
  for (const segment of segments) {
    if (position > segment.start && position < segment.end) {
      newSegments.push({ ...segment, end: position }, { ...segment, start: position + 1 });
    } else {
...
export const mergeSegments = (segments: Segments, position: number): Segments => {
  ...
  const newSegments = segments
    .filter((s) => s.start !== targetSegment.start && s.start !== neighbourToEat.start)
...
export const shiftSegment = (
  ...
  if (segmentIdx !== 0) {
    // clamp to left border: avoid eating up left neighbour
  ...
  const newSegments = segments
    .filter(
      (s) =>
        s.start !== oldSegmentStart &&
        s.start !== leftNeighbour?.start &&
        s.start !== rightNeighbour?.start,
    )
