...
describe("segment merging", () => {
    it("merges two adjacent segments", () => {
        const segments: Segments = [
            { id: "1", start: 0, end: 5, value: { displayName: "A", numericValue: 3 } },
            { id: "2", start: 6, end: 10, value: { displayName: "B", numericValue: 4 } },
            { id: "3", start: 11, end: 15, value: { displayName: "C", numericValue: 5 } },
        ];
...
describe("segment validation", () => {
    it("validates that segments fill the complete range", () => {
        ...
        const segmentsWrong1: Segments = [
            { id: "1", start: 0, end: 5, value: { displayName: "A", numericValue: 3 } },
            { id: "2", start: 6, end: 8, value: { displayName: "B", numericValue: 5 } },
        ];
    ...
    it("validates that segments have start <= end", () => {
        const segments: Segments = [
            { id: "1", start: 0, end: 5, value: { displayName: "A", numericValue: 3 } },
            { id: "2", start: 10, end: 6, value: { displayName: "B", numericValue: 5 } },
        ];
        expect(areSegmentsValid(segments, 0, 10)).toBe(false);
...
describe("segment shifting", () => {
    it("can not shift a single-segment-range", () => {
        const segments: Segments = [
            { id: "1", start: 10, end: 15, value: { displayName: "A", numericValue: 3 } },
        ];
        const expandRight = shiftSegment(segments, "1", 10, 16);
        expect(expandRight).toEqual(segments);

        const expandLeft = shiftSegment(segments, "1", 9, 15);
        expect(expandLeft).toEqual(segments);

        const expandBoth = shiftSegment(segments, "1", 9, 16);
