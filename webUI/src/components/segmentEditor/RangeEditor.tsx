...
const SegmentBody = ({
  ...
  right: number;
  input: InputOutput;
  onSegmentClick: (segmentStart: number, event: React.MouseEvent<SVGRectElement>) => void;
}) => {
...
export default function RangeValueAxisEditor({ input, output, segments, setSegments }: Props) {
  ...
  const updateCurrentSegmentOutput = (value: NamedValue) => {
    console.log(
      "Updating segment output value for segment",
      menuState.currentSegmentStart,
      "to",
      value,
      "menuzState",
      menuState,
  ...
  const setSegmentOutputValue = (segmentStart: number, value: NamedValue) => {
    ...
    setSegments(segments.map((s) => (s.start === segmentStart ? { ...s, value } : s)));
  };
  ...
  return (
    <Root ref={containerRef}>
      ...
      <Axis onClick={handleAxisClick}>
        ...
        {input.values
          ...
          .map((value) => {
            ...
            return (
              <AxisLabel key={value.numericValue} sx={{ left: `${leftPercent}%` }}>
                {value.displayName}
              </AxisLabel>
            );
          })}
      ...
      <Menu
        ...
        anchorPosition={
          menuState.mouse ? { top: menuState.mouse.y, left: menuState.mouse.x } : undefined
        }
      ...
      >
        ...
        >
          <Box sx={{ typography: "body2", color: "text.secondary" }}>
            ...
            {getDisplayName(getActiveSegment()?.end)}, set {output.displayName} to
          </Box>

          <InputValueEditor
            includeIgnore
            label="Output"
            value={getActiveSegment()?.value?.numericValue ?? output.min}
            input={output}
