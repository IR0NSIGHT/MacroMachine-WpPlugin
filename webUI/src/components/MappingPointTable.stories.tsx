...
const toMappingPoint = (
  ...
  return {
    ...
    output: output,
    y:
      (value.numericValue % 3) + 1 == 0
        ? output.ignoreValue
        : output.values[Math.abs(value.numericValue + 5) % output.values.length].numericValue,
  };
};
...
export const Default: Story = {
  render: (args) => {
    ...
    return <MappingPointTable {...args} points={points} setPoints={setPoints} />;
  },
  args: {
    points: annotationsIO.values
      .filter((v) => v.numericValue != annotationsIO.ignoreValue)
      .map((val) => toMappingPoint(val, annotationsIO, forestIO)),
    setPoints: (newPoints) => {
...
export const Filter: Story = {
  ...
  args: {
    points: annotationsIO.values
      .filter((v) => v.numericValue != annotationsIO.ignoreValue)
      .map((val) => toMappingPoint(val, annotationsIO, filterIO)),
    setPoints: (newPoints) => {
      console.log(" set points to: ", newPoints);
    },
  },
...
export const ManyPoints: Story = {
  ...Default,
  args: {
    points: heightIO.values
      .filter((v) => v.numericValue != annotationsIO.ignoreValue)
      .map((val) => toMappingPoint(val, heightIO, annotationsIO)),
    setPoints: (newPoints) => {
      console.log(" set points to: ", newPoints);
    },
  },
