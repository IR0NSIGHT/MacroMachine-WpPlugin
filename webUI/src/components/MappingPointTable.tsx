...
export const MappingPointTable = ({ points, setPoints }: Props) => {
  ...
  if (points.some((p) => !isInputOutput(p.input) || !isInputOutput(p.output))) {
    console.error(
      "Invalid data:",
      JSON.stringify(points.filter((p) => !isInputOutput(p.input) || !isInputOutput(p.output))),
    );
  } else {
  ...
  const [sortKey, setSortKey] = useState<"input" | "output">(
    points[0].input.discrete ? "output" : "input",
  ); // discrete inputs care about output first, continuous inputs care about input first
  const [sortDirection, setSortDirection] = useState<"asc" | "desc">(
    points[0].input.discrete ? "desc" : "asc",
  ); //ignore values are the first in asc
  ...
  const filteredRows = rows
    ...
    .filter((row) => (hideIgnoreValues ? row.y !== row.output.ignoreValue : true));
