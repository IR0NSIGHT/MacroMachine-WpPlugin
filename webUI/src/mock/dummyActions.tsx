...
const mappingPointsRaiseYonCyan: MappingPointDTO[] = inputValues(annotationsIO).map(p => ({ x: p.numericValue, y: p.numericValue != 9 ? 3 : heightIO.ignoreValue }));
...
export const slopeToForest: MMAction = {
  ...
  mappedOutputs: inputValues(slopeIO).map(v => v.numericValue <= 30 ? 0 : (v.numericValue <= 60 ? Math.round(((v.numericValue - 30) / (60-30)) * 7) : 7)), // flat 0 till 30, itnerpolate to y=50 till x=60, flat till end
  mappingPoints: [{ x: 30, y: 0 }, { x: 60, y: 7 }],
...
export const slopeToTerrain: MMAction = {
  ...
  actionType: "sets",
  mappedInputs: inputValues(slopeIO).map(v => v.numericValue),
  mappedOutputs: inputValues(slopeIO).map(v => v.numericValue < 30 ? 0 : (v.numericValue < 60 ? 3 :  7)), // flat 0 till 30, itnerpolate to y=50 till x=60, flat till end
  mappingPoints: [{ x: 30, y: 3 }, { x: 60, y: 7 }],
...
const mappingPointsOnlyOnLand: MappingPointDTO[] = inputValues(waterdepthIO).map(p => ({ x: p.numericValue, y: p.numericValue != 0 ? 0  /*block */ : -1 /** ignore */ }));
...
const mappingPointsOnlyOnCyan: MappingPointDTO[] = inputValues(annotationsIO).map(p => ({ x: p.numericValue, y: p.numericValue != 9 ? 0 : -1 }));
